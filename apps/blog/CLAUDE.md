<!-- BWF: v0.1 -->
# CLAUDE.md — apps/blog (blog.keeply.work)

> Hugo-based multilingual blog (19 languages) for `blog.keeply.work`. Part of the `keeply-suite` monorepo.
...
- **Source**: `apps/blog/` inside `keeply-suite` monorepo
- **Sister app**: `apps/website/` (keeply.work apex) — see [../website/CLAUDE.md](../website/CLAUDE.md)
- **BWF specs（寫作產物）**: 本地 `apps/blog/specs/{slug}/`（gitignored；推送目的地 `Z:\keeply-blog\`，使用者手動同步）
...
## App Boundary (intra-monorepo, replaces former cross-repo rule)

After the 2026-05-19 merge into `keeply-suite`, the "Cross-Repo Boundary" rule changed shape:

- Writes inside **`apps/blog/`** = OK from any session in `keeply-suite`
- Writes inside **`apps/website/`** = OK from any session in `keeply-suite` — coordinate via root CLAUDE.md `Cross-app URL refs` rule
- Writes to **other repos** still OFF-LIMITS:
  - `d:\tools\doing\Keeply\` (Tauri desktop app — Keeply 自帶的 `website/` 是舊 landing page，不要碰) — **絕對禁寫**
  - `d:\tools\doing\Ocular\` 等其他 — **絕對禁寫**

**Historical incident (kept for context)**：2026-05-02 keeply-blog 的某 session 把 `translation-template.md` 翻譯模板寫進了 `d:\tools\doing\Keeply\website\i18n\`（已加入 Keeply 的 `.gitignore` 排除）。Cross-repo 越界 — 翻譯模板若是給網站用的，應該寫進 `apps/website/` (post-merge) 或 `keeply-website` repo (pre-merge)，不該流到 Keeply 桌面 app repo。

**Self-check before write**：path 開頭必須是 `D:/tools/doing/keeply-suite/` 或 `~/.claude/bwf/`（user-level framework）。其他絕對 path 的寫入都要先停下來問 user。
...
**前置**：在 `apps/blog/` 目錄（`D:/tools/doing/keeply-suite/apps/blog/`）執行；hugo.toml `baseURL = "https://blog.keeply.work/"`、`defaultContentLanguageInSubdir = true`、theme `hugo-theme-stack`（git submodule，未 vendored）。
...
**鏈條描述**（reverse-engineered from `D:/tools/doing/keeply-suite/apps/blog/.github/workflows/deploy.yml`）：
