...
延續第 7 節 master 規則：zh-TW master + 全 6 locale draft 填齊（ja / ko / it 上線前建議走 native polish pass 校對自然度）。檔名字串保留 v6/v7/FINAL 等英數結構。

| Article × Image | zh-TW (master) | en | zh-CN | ja | ko | it |
|---|---|---|---|---|---|---|
| Shared-folders #1 file names | `Floorplan_v6.dwg / Floorplan_v7_Client.dwg / Floorplan_v7_FINAL_千萬別動.dwg / Floorplan_v8_送審版_0423.dwg / Floorplan_v8_真的最終.dwg / Floorplan_v8_FINAL_NEW.dwg / Floorplan_v8_老闆改完.dwg` | `Floorplan_v6.dwg / Floorplan_v7_Client.dwg / Floorplan_v7_FINAL_DO_NOT_TOUCH.dwg / Floorplan_v8_submission_0423.dwg / Floorplan_v8_really_final.dwg / Floorplan_v8_FINAL_NEW.dwg / Floorplan_v8_boss_edited.dwg` | `Floorplan_v6.dwg / v7_Client / v7_FINAL_千万别动 / v8_送审版_0423 / v8_真的最终 / v8_FINAL_NEW / v8_老板改完` | `Floorplan_v6.dwg / v7_Client / v7_FINAL_絶対触るな / v8_提出版_0423 / v8_本当に最終 / v8_FINAL_NEW / v8_部長修正済` | `Floorplan_v6.dwg / v7_Client / v7_FINAL_절대건드리지마 / v8_제출버전_0423 / v8_진짜최종 / v8_FINAL_NEW / v8_상사수정완료` | `Floorplan_v6.dwg / v7_Client / v7_FINAL_NON_TOCCARE / v8_invio_0423 / v8_davvero_finale / v8_FINAL_NEW / v8_capo_modificato` |
| Shared-folders #2 title | `知識工作者 1 年花在「關於工作的工作」` | `Hours/year spent on "work about work"` | `知识工作者 1 年花在「关于工作的工作」` | `ナレッジワーカーが「仕事についての仕事」に費やす時間／年` | `지식 근로자가 "일에 대한 일"에 1 년에 쓰는 시간` | `Ore/anno spese in "work about work"` |
| Shared-folders #2 callout | `83 小時 ≈ 2 個工作週` | `83 hours ≈ 2 work weeks` | `83 小时 ≈ 2 个工作周` | `83 時間 ≈ 2 営業週間` | `83 시간 ≈ 2 영업 주` | `83 ore ≈ 2 settimane lavorative` |
| Shared-folders #3 timeline | `Week 1 全部門很乖 / Week 6 有人趕件存 _NEW / Week 12 資料夾變回垃圾山` | `Week 1 Team disciplined / Week 6 Someone rushed, saves _NEW / Week 12 Folder back to garbage mountain` | `Week 1 全部门很乖 / Week 6 有人赶件存 _NEW / Week 12 文件夹变回垃圾山` | `Week 1 全員守ってる / Week 6 締切組が _NEW で保存 / Week 12 フォルダがゴミの山に逆戻り` | `Week 1 모두 잘 따름 / Week 6 급한 사람이 _NEW 로 저장 / Week 12 폴더 다시 쓰레기 산` | `Sett. 1 Tutti disciplinati / Sett. 6 Qualcuno di fretta salva _NEW / Sett. 12 Cartella torna spazzatura` |
| Shared-folders #3 底註 | `紀律會被趕件擊穿，機制不會` | `Discipline buckles under deadline pressure. Mechanism doesn't.` | `纪律会被赶件击穿，机制不会` | `規律は締切に屈する。仕組みは屈しない。` | `규율은 마감에 무너진다. 메커니즘은 무너지지 않는다.` | `La disciplina cede sotto pressione. Il meccanismo no.` |
| Shared-folders #4 左欄 | `Floorplan_v8_FINAL_千萬別動 ×8` | `Floorplan_v8_FINAL_DO_NOT_TOUCH ×8` | `Floorplan_v8_FINAL_千万别动 ×8` | `Floorplan_v8_FINAL_絶対触るな ×8` | `Floorplan_v8_FINAL_절대건드리지마 ×8` | `Floorplan_v8_FINAL_NON_TOCCARE ×8` |
| Shared-folders #4 右欄 | `Floorplan.dwg + Keeply 時間軸 8 版本` | `Floorplan.dwg + Keeply timeline 8 versions` | `Floorplan.dwg + Keeply 时间轴 8 版本` | `Floorplan.dwg + Keeply タイムライン 8 バージョン` | `Floorplan.dwg + Keeply 타임라인 8 버전` | `Floorplan.dwg + timeline Keeply 8 versioni` |
| ─ | ─ | ─ | ─ | ─ | ─ | ─ |
| Recover #1 5 cards | `AutoRecover / OneDrive 版本歷史 / Windows 以前的版本 / 資料復原軟體 / Time Machine` | `AutoRecover / OneDrive Version History / Windows Previous Versions / Data Recovery Software / Time Machine` | `AutoRecover / OneDrive 版本历史 / Windows 以前的版本 / 数据恢复软件 / Time Machine` | `AutoRecover / OneDrive バージョン履歴 / Windows 以前のバージョン / データ復元ソフト / Time Machine` | `AutoRecover / OneDrive 버전 기록 / Windows 이전 버전 / 데이터 복구 SW / Time Machine` | `AutoRecover / Cronologia OneDrive / Versioni precedenti Windows / Software recupero dati / Time Machine` |
| Recover #1 ✅❌ label | `救得到 ✅ / 救不到 ❌` | `Covers ✅ / Doesn't ❌` | `救得到 ✅ / 救不到 ❌` | `カバー ✅ / カバー外 ❌` | `복구 가능 ✅ / 복구 불가 ❌` | `Coperto ✅ / Non coperto ❌` |
| Recover #2 X 軸 | `覆蓋後時間 — 0 / 5 min / 30 min / 1 hr / 1 day / 1 week` | `Time after overwrite — 0 / 5 min / 30 min / 1 hr / 1 day / 1 week` | `覆盖后时间 — 0 / 5 min / 30 min / 1 hr / 1 day / 1 week` | `上書き後経過時間 — 0 / 5 分 / 30 分 / 1 時間 / 1 日 / 1 週間` | `덮어쓰기 후 시간 — 0 / 5 분 / 30 분 / 1 시간 / 1 일 / 1 주` | `Tempo dopo sovrascrittura — 0 / 5 min / 30 min / 1 h / 1 g / 1 sett` |
| Recover #2 Y 軸 | `救回成功率` | `Recovery success rate` | `救回成功率` | `復元成功率` | `복구 성공률` | `Tasso di successo` |
| Recover #2 曲線 label | `AutoRecover（關檔即 0）/ Recovery SW HDD（緩降）/ Recovery SW SSD TRIM（陡降）/ Keeply 事前防禦（持平 100%）` | `AutoRecover (drops to 0 on close) / Recovery SW HDD (gradual) / Recovery SW SSD TRIM (steep drop) / Keeply pre-emptive (flat 100%)` | `AutoRecover（关档即 0）/ Recovery SW HDD（缓降）/ Recovery SW SSD TRIM（陡降）/ Keeply 事前防御（持平 100%）` | `AutoRecover（閉じた瞬間 0）/ 復元 SW HDD（緩降下）/ 復元 SW SSD TRIM（急降下）/ Keeply 事前防御（100% 横ばい）` | `AutoRecover（닫는 즉시 0）/ 복구 SW HDD（완만 하락）/ 복구 SW SSD TRIM（급락）/ Keeply 사전 방어（100% 유지）` | `AutoRecover (zero alla chiusura) / Software HDD (calo graduale) / Software SSD TRIM (calo rapido) / Keeply preventivo (100% costante)` |
| Recover #2 底註 | `事前防禦不依賴發現的時機` | `Pre-emptive protection doesn't depend on when you notice` | `事前防御不依赖发现的时机` | `事前防御は気づくタイミングに依存しない` | `사전 방어는 발견 시점에 의존하지 않는다` | `La protezione preventiva non dipende da quando te ne accorgi` |
| Recover #3 左欄 | `A 先生（沒事前防禦）— 19:30 覆蓋 → AutoRecover ❌ → Recuva ❌ → 剩 60 hr 焦慮` | `Mr. A (no pre-emptive) — 19:30 overwrite → AutoRecover ❌ → Recuva ❌ → 60 hr panic` | `A 先生（没事前防御）— 19:30 覆盖 → AutoRecover ❌ → Recuva ❌ → 剩 60 hr 焦虑` | `A さん（事前防御なし）— 19:30 上書き → AutoRecover ❌ → Recuva ❌ → 残り 60 時間の焦り` | `A 씨（사전 방어 없음）— 19:30 덮어쓰기 → AutoRecover ❌ → Recuva ❌ → 남은 60 시간 불안` | `Sig. A (senza prevenzione) — 19:30 sovrascrittura → AutoRecover ❌ → Recuva ❌ → 60 ore di panico` |
| Recover #3 右欄 | `B 小姐（用 Keeply）— 週一發現 → 開 Keeply → 19:00/19:15/19:30 三版本 → 點 19:00 → 3 秒還原 ✅` | `Ms. B (with Keeply) — Monday notice → Open Keeply → 19:00/19:15/19:30 three versions → click 19:00 → 3 sec restore ✅` | `B 小姐（用 Keeply）— 周一发现 → 开 Keeply → 19:00/19:15/19:30 三版本 → 点 19:00 → 3 秒还原 ✅` | `B さん（Keeply 使用）— 月曜に気づく → Keeply を開く → 19:00/19:15/19:30 の 3 バージョン → 19:00 をクリック → 3 秒で復元 ✅` | `B 씨（Keeply 사용）— 월요일 발견 → Keeply 열기 → 19:00/19:15/19:30 세 버전 → 19:00 클릭 → 3 초 복원 ✅` | `Sig.ra B (con Keeply) — Lunedì se ne accorge → Apre Keeply → 3 versioni 19:00/19:15/19:30 → Click su 19:00 → Recupero in 3 sec ✅` |
| ─ | ─ | ─ | ─ | ─ | ─ | ─ |
| Deleted-list #1 上半 | `有清單 ✅ — iOS 照片 / iCloud Drive / 備忘錄 / Outlook / Gmail / Slack` | `Has list ✅ — iOS Photos / iCloud Drive / Notes / Outlook / Gmail / Slack` | `有清单 ✅ — iOS 照片 / iCloud Drive / 备忘录 / Outlook / Gmail / Slack` | `リストあり ✅ — iOS 写真 / iCloud Drive / メモ / Outlook / Gmail / Slack` | `목록 있음 ✅ — iOS 사진 / iCloud Drive / 메모 / Outlook / Gmail / Slack` | `Con elenco ✅ — Foto iOS / iCloud Drive / Note / Outlook / Gmail / Slack` |
| Deleted-list #1 下半 | `沒有清單 ❌ — Finder / 檔案總管 / Dropbox 本機 / Google Drive 本機` | `No list ❌ — Finder / File Explorer / Dropbox local / Google Drive local` | `没有清单 ❌ — Finder / 文件资源管理器 / Dropbox 本地 / Google Drive 本地` | `リストなし ❌ — Finder / エクスプローラー / Dropbox ローカル / Google Drive ローカル` | `목록 없음 ❌ — Finder / 파일 탐색기 / Dropbox 로컬 / Google Drive 로컬` | `Senza elenco ❌ — Finder / Esplora file / Dropbox locale / Google Drive locale` |
| Deleted-list #1 底註 | `Pattern 偏偏缺在你最需要的地方` | `The pattern is missing exactly where you need it most` | `Pattern 偏偏缺在你最需要的地方` | `必要なところに限ってこのパターンが欠けている` | `정작 필요한 곳에 이 패턴이 빠져 있다` | `Il pattern manca proprio dove ne hai più bisogno` |
| Deleted-list #2 左欄 | `Curated app — 跟內容互動 — 「最近刪除」= 基本元件` | `Curated app — interacting with content — "Recently Deleted" = essential element` | `Curated app — 跟内容互动 —「最近删除」= 基本组件` | `Curated app — コンテンツと対話 —「最近削除した項目」= 基本要素` | `Curated app — 콘텐츠와 상호작용 —「최근 삭제」= 기본 요소` | `App curated — interazione con contenuti — "Eliminati di recente" = elemento essenziale` |
| Deleted-list #2 右欄 | `File-system mirror — 透明反映磁碟 — 加面板違反契約` | `File-system mirror — transparent disk reflection — adding panel breaks contract` | `File-system mirror — 透明反映磁盘 — 加面板违反契约` | `ファイルシステムミラー — ディスクの透過的反映 — パネル追加は契約違反` | `파일 시스템 미러 — 디스크 투명 반영 — 패널 추가는 계약 위반` | `Mirror file-system — riflesso trasparente del disco — pannello viola il contratto` |
| Deleted-list #2 橋接 | `設計選擇 → 救援摩擦差異` | `Design choice → recovery friction gap` | `设计选择 → 救援摩擦差异` | `設計上の選択 → 復旧の摩擦の違い` | `설계 선택 → 복구 마찰 차이` | `Scelta di design → divario nell'attrito di recupero` |
| Deleted-list #3 三路徑 | `A: 有清單 → 1 鍵 → 5 秒 ✅ / B: 時間軸翻找 → 5 分鐘 ⚠️ / C: Disk Drill $89 → 2 小時 → SSD TRIM 可能失敗 ⏰❌` | `A: Has list → 1-click → 5 sec ✅ / B: Timeline search → 5 min ⚠️ / C: Disk Drill $89 → 2 hr → SSD TRIM might fail ⏰❌` | `A: 有清单 → 1 键 → 5 秒 ✅ / B: 时间轴翻找 → 5 分钟 ⚠️ / C: Disk Drill $89 → 2 小时 → SSD TRIM 可能失败 ⏰❌` | `A: リストあり → 1 クリック → 5 秒 ✅ / B: タイムライン探索 → 5 分 ⚠️ / C: Disk Drill $89 → 2 時間 → SSD TRIM 失敗の可能性 ⏰❌` | `A: 목록 있음 → 1 클릭 → 5 초 ✅ / B: 타임라인 탐색 → 5 분 ⚠️ / C: Disk Drill $89 → 2 시간 → SSD TRIM 실패 가능 ⏰❌` | `A: Con elenco → 1 click → 5 sec ✅ / B: Ricerca timeline → 5 min ⚠️ / C: Disk Drill $89 → 2 ore → SSD TRIM può fallire ⏰❌` |
| ─ | ─ | ─ | ─ | ─ | ─ | ─ |
| Photoshop #1 path | `~/Documents/Adobe/AutoRecover/` | `~/Documents/Adobe/AutoRecover/` | `~/Documents/Adobe/AutoRecover/` | `~/Documents/Adobe/AutoRecover/` | `~/Documents/Adobe/AutoRecover/` | `~/Documents/Adobe/AutoRecover/` |
| Photoshop #1 底註 | `今天的工作從來沒被寫進去` | `Today's work was never written here` | `今天的工作从来没被写进去` | `今日の作業はここに書き込まれていない` | `오늘의 작업은 여기에 기록된 적이 없다` | `Il lavoro di oggi non è mai stato scritto qui` |
| Photoshop #2 3 機制 title | `自動儲存 / 歷史紀錄面板 / 檔案層級版本歷史` | `Auto-Save / History Panel / File-level Version History` | `自动保存 / 历史记录面板 / 文件层级版本历史` | `自動保存 / ヒストリーパネル / ファイル単位のバージョン履歴` | `자동 저장 / 히스토리 패널 / 파일 단위 버전 기록` | `Salvataggio automatico / Pannello cronologia / Cronologia versioni a livello file` |
| Photoshop #2 觸發 label | `當機 / 每動作 / 每 Cmd+S` | `On crash / On each action / On each Cmd+S` | `当机 / 每动作 / 每 Cmd+S` | `クラッシュ時 / 操作ごと / Cmd+S ごと` | `크래시 시 / 동작마다 / Cmd+S 마다` | `Crash / Ad ogni azione / Ad ogni Cmd+S` |
| Photoshop #2 內建 label | `內建 ✅ ✅ ❌` | `Built-in ✅ ✅ ❌` | `内建 ✅ ✅ ❌` | `内蔵 ✅ ✅ ❌` | `내장 ✅ ✅ ❌` | `Integrato ✅ ✅ ❌` |
| Photoshop #2 救蓋掉版本 label | `救蓋掉版本 ❌ ❌ ✅` | `Saves overwritten versions ❌ ❌ ✅` | `救盖掉版本 ❌ ❌ ✅` | `上書き版を救う ❌ ❌ ✅` | `덮어쓴 버전 복구 ❌ ❌ ✅` | `Recupera versioni sovrascritte ❌ ❌ ✅` |
| Photoshop #3 左 | `Photoshop 歷史紀錄面板 — 20 個今早動作 — 檔案一關就蒸發` | `Photoshop History Panel — 20 actions this morning — vanishes when file closes` | `Photoshop 历史记录面板 — 20 个今早动作 — 文件一关就蒸发` | `Photoshop ヒストリーパネル — 今朝の 20 操作 — ファイルを閉じると消える` | `Photoshop 히스토리 패널 — 오늘 아침 동작 20 개 — 파일 닫으면 사라짐` | `Pannello cronologia Photoshop — 20 azioni di stamattina — svanisce alla chiusura del file` |
| Photoshop #3 右 | `Keeply 時間軸 — 6 個 Cmd+S 快照 — 永久保留 / 跨 session` | `Keeply timeline — 6 Cmd+S snapshots — persistent / cross-session` | `Keeply 时间轴 — 6 个 Cmd+S 快照 — 永久保留 / 跨 session` | `Keeply タイムライン — Cmd+S スナップショット 6 個 — 永続 / セッション横断` | `Keeply 타임라인 — Cmd+S 스냅샷 6 개 — 영구 보존 / 세션 간 유지` | `Timeline Keeply — 6 snapshot Cmd+S — persistente / tra sessioni` |
| Photoshop #4 上層 | `Adobe 內建層 — 每次 Cmd+S 覆蓋同一位置 — 無歷史` | `Adobe built-in — each Cmd+S overwrites same location — no history` | `Adobe 内建层 — 每次 Cmd+S 覆盖同一位置 — 无历史` | `Adobe 内蔵層 — Cmd+S ごとに同じ場所を上書き — 履歴なし` | `Adobe 내장 층 — Cmd+S 마다 같은 위치 덮어쓰기 — 기록 없음` | `Livello Adobe integrato — ogni Cmd+S sovrascrive stessa posizione — niente cronologia` |
| Photoshop #4 下層 | `Keeply 補的層 — 每次 Cmd+S 各留快照 — 30 分鐘前可還原` | `Keeply complement — each Cmd+S keeps a snapshot — restorable to 30 min ago` | `Keeply 补的层 — 每次 Cmd+S 各留快照 — 30 分钟前可还原` | `Keeply の補完層 — Cmd+S ごとにスナップショット保存 — 30 分前まで復元可` | `Keeply 보완 층 — Cmd+S 마다 스냅샷 보존 — 30 분 전까지 복원 가능` | `Livello complementare Keeply — ogni Cmd+S salva snapshot — recuperabile fino a 30 min fa` |
| Photoshop #4 橋接 | `兩層各管一段，並排運作` | `Two layers, each handling its own segment, side-by-side` | `两层各管一段，并排运作` | `2 つの層がそれぞれの担当領域を持ち、並走する` | `두 층이 각자 영역을 담당하며 병행 운영` | `Due livelli, ognuno il proprio segmento, in parallelo` |
...
| 標籤 | zh-TW master | en | zh-CN | ja | ko | it |
|---|---|---|---|---|---|---|
| Dropbox #1 底註 | `不是 bug，是設計` | `Not a bug, by design` | `不是 bug，是设计` | `バグではなく、設計` | `버그가 아니라, 설계` | `Non è un bug, è il design` |
| Excel #1 callout | `需要 4 個條件全成立——你一個都沒中` | `Requires all 4 conditions—you meet none` | `需要 4 个条件全成立——你一个都没中` | `4 つの条件すべて必要——あなたは 1 つも満たさない` | `4 가지 조건 모두 필요——당신은 하나도 충족 못함` | `Servono tutte e 4 le condizioni — non ne soddisfi nessuna` |
| Excel #2 中央 / 外圍 | `✅ 按鈕亮 / ❌ 多數人都在這裡` | `✅ Button enabled / ❌ Most people land here` | `✅ 按钮亮 / ❌ 多数人都在这里` | `✅ ボタン有効 / ❌ ほとんどの人はここ` | `✅ 버튼 활성 / ❌ 대부분 여기에 해당` | `✅ Pulsante attivo / ❌ Qui la maggior parte` |
| AutoCAD #1 斷點標籤 | `辦公室→工地 斷` | `Office → Site BROKEN` | `办公室→工地 断` | `オフィス → 現場 断絶` | `사무실 → 현장 단절` | `Ufficio → Cantiere INTERROTTO` |
| Client-Word #1 底註 | `3 月那版不在這裡` | `The March version isn't here` | `3 月那版不在这里` | `3 月のあの版はここにない` | `3 월의 그 버전은 여기에 없다` | `La versione di marzo non è qui` |
| Restore #1 底註 | `救援軟體掃描看到的只剩一片零` | `Recovery software scans see only zeros` | `救援软件扫描看到的只剩一片零` | `復旧ソフトのスキャンに映るのはゼロだけ` | `복구 소프트웨어 스캔에 보이는 건 0 뿐` | `Il software di recupero vede solo zeri` |
| Departing #1 底註 | `Dropbox 是同步不是備份` | `Dropbox is sync, not backup` | `Dropbox 是同步不是备份` | `Dropbox は同期であってバックアップではない` | `Dropbox 는 동기화이지 백업이 아니다` | `Dropbox è sync, non backup` |
| Why-built #1 ❌ 標籤 | `（這條線最容易斷）` | `(this link breaks most often)` | `（这条线最容易断）` | `（この線が一番切れやすい）` | `（이 선이 가장 자주 끊긴다）` | `(questa è la linea che si rompe più spesso)` |

6 locale draft 已填齊；ja / ko / it 上線前建議走 native polish pass 校對自然度（同 §7 + §17 SOP）。
