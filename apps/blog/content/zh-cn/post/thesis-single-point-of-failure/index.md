...
howto_schema:
  ...
  steps:
    ...
    - name: 交导师前独立留档
      ...
      url: '#thesis-4-step'
    - name: 工具自动记录每版差异
      text: 导入 Keeply 让你存的每一版自动留记录、可以写笔记（例如「3.2 节重写」「导师第三轮反馈后」），半年后直接看时间轴点开差异视图，不用手动翻 v1 v2 v3 文件名。
      url: '#thesis-4-step'
    - name: 至少一份不在这台笔电
...
点开 `thesis_v3.docx` 的版本历史面板、看到的是这 4 个月来导师每一轮反馈后的版本一路堆叠：
...
| **[Keeply](https://keeply.work)** | **你存的每一版自动记、可以写笔记、半年后翻时间轴看得到差异** | **整颗 SSD 物理坏掉（要搭 3-2-1）** |
