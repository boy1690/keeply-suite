...
你主动标「这版客户签了」、「这版上线了」、之后不论怎么改、冻结点还在。**例子**：GitHub Release（工程师圈把某个时间点的代码冻结成版本的功能、只给开发者用）。**Keeply** 内建一个叫「发行版」的功能（对应 ADR-003）、做同一件事但你不用学任何术语：在版本历史里选一版、按一下「冻结为发行版」、之后永远回得来。**解第 2 类痛点**。
