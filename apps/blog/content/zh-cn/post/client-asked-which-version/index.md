...
description: "Word 自动恢复 文件关闭就清、OneDrive 版本历史 500 版上限、Time Machine 30 天就回收——都是存储层救援。3 个月后客户问「3 月那版可以再传一份吗」？换 Keeply 后 4/12 业主核定版自己一行 tag、含 PDF 收执单、半年后翻时间轴 3 秒回。"
...
faq_schema:
  - q: Word 内建版本历史能做什么？
    a: Word 有三种机制：自动恢复（崩溃救援、关闭即清除）、自动保存（边打边存至云端）、OneDrive 版本历史（保留约 500 个版本快照）。三种都是短期存储事故救援、设计目标不包含 3 个月后的交付版本追踪。

  - q: 自动恢复、OneDrive 和 Time Machine 各能保留多久？
    a: 自动恢复 在文件正常关闭后即清除；OneDrive 版本历史预设约 500 个版本、超过自动删最旧；Mac Time Machine 每小时快照保留 24 小时、每天快照保留 30 天。每种机制都有保留上限、无法跨越 3 个月这条线。

  - q: 为什么 Word 版本历史守不到 3 个月后？
    a: 软件内建版本历史活在「存储层」、设计给最近一次写入失败用、保留期依平均用户一个月内查找频率设定。3 个月以上不在设计目标、清除是合理行为。需要工具层独立的常驻版本历史才能解决。
  ...
  - q: Keeply 适合所有 Word 用户吗？
    a: 不适合。Keeply 不取代 自动恢复（打到一半崩溃仍要靠 Word 本身）、不取代 Microsoft 365 共同编辑（5 人同时改文档走 Microsoft 365 / Google Docs 比较顺）、不能溯及既往（没装 Keeply 过的旧交付 Keeply 救不了）。但装了之后、从今天起的每次交付都救得了。
---
...
4. [为什么这些机制守不到 3 个月后？存储层 vs 工具层的设计差](#存储-vs-tool-layer)
...
## 我是怎么找到那一版的？Keeply diff 比对两个版本一眼看出差异

打开「4/12 业主核定版」跟「5/4 pricing 修订版」并排，Keeply 标出哪几段被改、改了什么数字：

![Keeply 版本差异检视：4/12 业主核定版 vs 5/4 pricing 修订版 + proposal.docx 32 行差异 + Basic $99→$149 / Pro $299→$399](diff-viewer.svg)

两段红绿对照、原本的 pricing 跟修订后的 pricing 一目了然。客户看到这张图就知道哪边改了——不用我口头说。
...
| **[Keeply](https://keeply.work)** | **无时间上限**（本机 git 不过期） | **用户主动删除才清** | **长期交付追踪** |

对啊、每个内建机制都有上限。文件关闭清除到 500 个版本、跨不过 3 个月这条线。3-2-1 防的是磁带腐坏、Word 防的是崩溃、这些都不是同一个问题。

---

## 为什么这些机制守不到 3 个月后？存储层 vs 工具层的设计差 {#存储-vs-tool-layer}

这里要拆一个没人明讲的差别：**存储层** vs **工具层**。

软件内建的版本历史活在**存储层**。它存在的目的是「最近一次写入失败就回滚」、所以保留期设得短。从文件关闭清除到 500 个版本上限、这些设计参考的是「平均用户一个月内回头找的次数」。3 个月以上不在设计目标内、清除掉是合理的。
...
**常驻版本历史**——你存的每一版都留下、不会清除。不依赖 OneDrive 订阅、不依赖 AutoSave、文件存桌面也照存。本机 git 没时间上限、500 版本上限不存在。
