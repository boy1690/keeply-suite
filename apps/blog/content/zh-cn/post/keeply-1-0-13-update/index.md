...
![删除时光机：左侧依日期列出每一版删了什么，右侧时光机文件树标出被删文件，勾选后一键还原](deletion-time-machine.png)

### 点一个文件，只看它的版本历史

时间轴一直以来显示的是整个文件夹的版本。但很多时候你只想知道「就这一份文件，到底改过哪几版」。现在直接单击那个文件，左边的主时间轴就只剩这份文件的历史；看完按一下「回到全部版本」就回到完整时间轴。

![单击一个文件，主时间轴自动只显示这份文件的版本历史](file-timeline.png)
...
![团队协作抢先看：团队活动地铁图，主干为主版本、各成员工作副本为支线，标出合并与审阅状态（功能尚未开放）](team-metro-map.png)
