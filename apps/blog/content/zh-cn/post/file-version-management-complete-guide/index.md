...
faq_schema:
  - q: 什么是文件版本管理？
    a: 文件版本管理，就是把一个文件先后的每一个版本都留下来，还顺手记下每次改了什么、为什么改——这样你之后想回去翻任何一个旧版本，不只是把旧文件找回来，还看得到当时那个决定。它跟备份不一样：备份只是帮你留一份副本；版本管理是连文件带它完整的修改历程一起留着。对于放在你自己电脑、或公司共享硬盘上的文件，像 Keeply 这类专门工具让你改到一个段落就随手按「存一版」、加一句备注，也能开启每 15 / 30 / 60 分钟的自动排程，不必再靠 _v2_final 这种手动命名硬撑。
  - q: 文件版本管理和备份差在哪？
    a: 备份是帮你留一份副本，以免文件整个丢失——不管是硬盘坏掉、中了勒索软件、还是被偷。版本管理则是连同文件，把每一次改了什么、为什么改，按时间顺序排好一起留着，让你能回到某个特定的旧版本，还知道它当时为什么变成那样。备份回答的是「我的文件还在吗」。版本管理回答的是「哪一版才对、到底改了什么」。多数人其实两件事都需要：一份安全的副本来应付灾难，再加上完整的修改历程来应付每天那种「这是谁覆盖掉的？」。能两件一起做的工具不多——Keeply 就是把同一个文件在多个位置各留一份，同时把完整的版本历程一起留着。
  - q: 为什么共享文件夹里找不到「上一版」按钮？
    a: 云盘的设计目标是让多台电脑看到同一份最新文件，「保留所有旧版」与「最新版同步」会打架，所以工具选了同步。版本历史被藏在三层菜单之外，或根本不存在，因为这从来不是这类工具的核心设计目标。
  - q: Dropbox 的 30 天版本历史限制是技术做不到还是商业决定？
    a: 纯粹是商业决定。macOS Time Machine 2007 年就示范了完整的本机版本历史，技术早已成熟。Dropbox 把版本历史设计为升级付费的差异化功能，30 天是商业设计，不是技术上限。
  - q: 版本历史只告诉你「何时」修改，为什么看不到「为何」修改？
    a: 现有版本历史界面只显示日期加用户，不显示改动的意图。设计师改了图层透明度、律师改了一个字的含义、研究生把谨慎改成武断，版本历史只记录「有修改」，不记录那次改动代表什么。
  - q: 为什么命名规则 PDF 通常 6 个月后就没人遵守？
    a: 命名规则把版本管理的责任推给人类纪律。每个成员每次保存都必须记得、愿意且有时间照规则命名。任何一条失败就崩溃。赶件压力下人脑只会本能输入 FINAL，机制问题被包装成纪律问题。
  - q: 什么时候 Keeply 不是版本管理的正确答案？
    a: 四种场景 Keeply 不适合：即时协作会议笔记用 Notion 或 Google Docs；50GB 以上视频素材用 Frame.io；对外法务签核用 DocuSign；以及大企业 IT 管控需求。Keeply 适合个人到小团队的长期知识工作版本记忆。
---
...
而且这不只是三个倒霉的人。在 [M-Files 2019 年那份针对知识工作者的调查里，96% 的人都说自己曾经为了找到一个文件的最新版本而碰过麻烦](https://idm.net.au/article/0012311-8-10-workers-forced-recreate-existing-documents)。几乎每个人都在默默和同一件事较劲。
...
举个例子，当你在版本史看到「proposal.docx 改了」，光看时间戳猜不出意义。把两版摊开：

![Keeply 版本比对：proposal.docx 第 3 节 L42 年费从 ¥ 168,000 改成 ¥ 199,500、附注「客户要 SLA」；同步新增 sla_addendum_v2.docx](diff-viewer.svg)

L42 年费 168,000 → 199,500，旁边看到「客户要 SLA」、同步多了一份 `sla_addendum_v2.docx`——三秒看懂这版为什么跳价。版本管理不是事后对照，是把当下的决策记在当下。
...
而当这锅粥赢了，你不是把旧版本找回来——你是从头重做一遍。同一份 [M-Files 调查发现，全球有 83% 的工作者被迫重做某个其实早就存在于某处的文件](https://idm.net.au/article/0012311-8-10-workers-forced-recreate-existing-documents)。那个版本一直都在。只是没人找得到。
