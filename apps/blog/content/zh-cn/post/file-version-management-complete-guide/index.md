...
faq_schema:
  - q: 什么是文件版本管理？
    a: 文件版本管理，就是把一个文件每次保存的版本都留下来，还顺手记下每次改了什么、为什么改——这样你之后想回去翻任何一个旧版本，不只是把旧文件找回来，还看得到当时那个决定。它跟备份不一样：备份只是帮你留一份副本；版本管理是连文件带它完整的修改历程一起留着。对于放在你自己电脑、或公司共享硬盘上的文件，像 Keeply 这类专门工具会在后台自动帮你留版本，也能让你随手存一版、加一句备注，不必再靠 _v2_final 这种手动命名硬撑。
  - q: 文件版本管理和备份差在哪？
