...
faq_schema:
  ...
  - q: Dropbox 为什么这样设计冲突副本机制？
    a: Dropbox 采用 last-writer-wins 策略：后上传的版本胜出，前一版另存为冲突副本。这是商业取舍，优先保障同步不打断工作流、而非做冲突检测。冲突解析责任被刻意推给用户，不是技术做不到。
...
Dropbox 用「后存者覆盖、前一版另存」这个机制（[Dropbox 官方说明](https://help.dropbox.com/organize/conflicted-copy)）：两人同时改、后上传的版本胜出、前一版不丢掉、存成 `(conflicted copy)`。
...
对啊、这就是让人烦的地方。Dropbox 把工具该做的事（冲突检测 + 提示）推给用户纪律。而纪律永远赢不过自动化。

---

## 手动合并两份文件为什么只是症状治疗？ {#why-manual-merge-fails}

[Dropbox Help Center](https://help.dropbox.com/organize/conflicted-copy) 教你的修法：「打开两份文件、比对差异、手动合并到主档、删掉冲突副本。」一听很合理。
...
两端改同档、同步时检测冲突、跳界面提示给用户选：留 A、留 B、或把两个变更合并。**例子**：工程师圈用的版本控制工具用这种模式。**Keeply** 把同样的检测搬进办公室工具：撞到冲突时、用「Anna 的版本」「你的版本」这种白话让你选、不会跳出术语。

实际长这样——Anna 那边也推了一版进保管库、Keeply 把「套用她的变更」这件事跳成对话框让你决定：

![Keeply 套用 Anna 变更对话框：版本来源 + 讯息「加 3 段背景脉络」+ 冲突处理选项说明](cherry-pick-dialog.svg)

点「套用」前 Keeply 会自动先把目前的版本拍一张快照（即使按错也能 Undo）、按下去之后若两边改到同一段文字、会跳第二层选择：保留你的 / 用 Anna 的 / 两个都留。**解场景 #1 + #2**。
