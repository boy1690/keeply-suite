...
faq_schema:
  - q: iCloud、Dropbox、OneDrive、Google Drive 4 家都有版本历史吗？
    a: 4 家中只有 3 家有通用档版本历史。iCloud Drive 不对外暴露 PSD / Word / PDF 等非 Apple 原生档的版本历史，只有 Pages、Numbers、Keynote 有内置版本浏览器。Dropbox、OneDrive、Google Drive 都保留版本，但 上限 形状不同。
  - q: Dropbox 保留文件版本几天？
  ...
  - q: OneDrive 的 500 版本上限跟 Dropbox 的 30 天窗口差别在哪？
    a: OneDrive 用版本数 上限——保留 500 个 主要版本，超过后最旧的会被删。Dropbox 用时间 上限——超过 30 天的版本一律消失，不管版本数多少。前者意味活跃档可能一周内就丢掉旧版，后者意味闲置档在 30 天时即使只有 5 个版本也会消失。
  ...
  - q: Keeply 是云端存储的替代品吗？
    a: 不是。Keeply 是云端同步之上的另一层工具——本地保留你存的每一版，无时间限制、无计数限制。云端负责同步跟异地副本，Keeply 负责版本历史。你不需要换工具，是叠一层上去。
---
...
Reframe 一下：你不换云端就能解这个问题。你的云端做同步没问题。少的那一块是**另一层**——文件层级的版本历史，无时间上限、你存的每一版都留（重要时刻手动保存版本，或开后台自动保存每 15/30/60 分）。

具体说：

- **云端(4 家任一)**负责同步 + 异地副本
- **版本历史层(Keeply 或同类)**负责留下你存的每一版（手动保存＋可选每 15/30/60 分自动），无时间 上限、无计数 上限、不需要在保存当下决定要不要「Keep forever」
...
Keeply 是这层的 reference 实现：你存的每一版本地保留，无时间 上限、无计数 上限，加上「Release」冻结机制——把某个版本标成「这版送给客户」，那个 快照 永远存在，后续保存五十次也盖不掉。两个月前的版本，回溯约 2 个点击。
