...
faq_schema:
  - q: 工地拿到的图到底是不是最新版，怎么最快确认？
    a: 不要靠文件名「FINAL_v7」判断。最快的办法是办公室和工地共用同一条版本时间轴——Keeply 在新版进办公室那一刻自动记录，工地端打开时间轴就看得到「2 小时前 第 5 版 — 盖板规格改」，不用 WhatsApp 问来问去。
  - q: 为什么文件夹里总是 _FINAL_v7、_really_final_v9？
    a: 因为手动命名没有强制规则，每个人定义「final」都不同。设计覆盖旧图时又怕弄丢旧版本，就一直加后缀。真正的解法是把版本记录交给工具，而不是逼每个人记住命名公约。
  - q: 图纸版本管理 4 步具体在做什么？
    a: 第 1 步：新版一进办公室立刻通知现场且回复「收到」。第 2 步：覆盖旧版前单独保留并标记版次。第 3 步：导入 Keeply 等工具自动记录每一版且能写笔记。第 4 步：至少一份异地备份，防止 NAS 故障。
  - q: 设计改回上周的旧版本怎么办？
    a: 这在工程行业很常见——业主说「rev 5 太花、回到 rev 2 那种收边」。如果旧版没留档，现场已经放样就麻烦了。Keeply 自动保留每一版且有笔记，回头点开「业主回 rev 2 之前」那一笔，整份还原就好。
  - q: 工地师傅需要每个人都装 Keeply 吗？
    a: 不用。Keeply 在办公室那台电脑记录整条版本历史，工地用印刷蓝图或现场 PM 一支设备确认就够。Keeply 解的是「办公室和现场版本对不上」这条缝，不是每个师傅手机都要看时间轴。
howto_schema:
  ...
  steps:
    ...
    - name: 新版覆盖旧版前先留档
      ...
      url: '#autocad-4-step'
    - name: 工具自动记录版本供全员查看
      text: 导入 Keeply 等版本管理工具，让你存的每一版自动记录、可以写笔记（如「盖板规格改 — 设计第 4 版」），所有人开启同一保管库即可看到同一条版本时间线。
      url: '#autocad-4-step'
    - name: 保留一份异地备份
...
而且打错版本施工很少只发生一次。多项研究显示，[返工占总工程成本 5–10%——PlanRadar 2023 年的数据是 7.9%](https://www.planradar.com/us/cost-of-rework-construction/)，而「照着没人提醒的修订版盖下去」正是其中最能避免的成因之一。
...
Keeply 在这里能做的事、就是让你能直接比两版图差在哪——不用开两个 AutoCAD 窗口肉眼扫：

![Keeply 图档差异对比：A-05_水沟.dwg v3 工地核定版对 v5 最新版、盖板规格从 12mm 改 15mm + 钢筋间距 200mm 改 150mm](diff-viewer.svg)

点 v3 跟 v5 两个版本、Keeply 列出两版差了哪几个图层、哪些尺寸改了。盖板从 12mm 铸铁变成 15mm、钢筋间距从 200mm 缩到 150mm——这是技师复核后加的。你看完 30 秒就知道现场该动哪边、不用打电话回办公室问。
...
**四、一份不在办公室、不在工地 NAS。** 外接硬盘、云端、备份槽、哪个都行。重点是**至少一份异地**（[CISA 的 3-2-1 备份原则](https://www.cisa.gov/audiences/small-and-medium-businesses/secure-your-business/back-up-business-data)：3 份副本、2 种媒体、1 份异地）。公司 NAS 坏了、被清了、被接手的人用掉、你回不来。异地备份是你给自己买的最便宜的保险。
