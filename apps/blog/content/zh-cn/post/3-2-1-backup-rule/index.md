...
faq_schema:
  ...
  - q: NAS 算 2 种媒介吗？
    a: NAS 加本机硬盘可算 2 种媒介。但 RAID 不算备份——RAID 防的是硬盘坏掉、不防你自己删错。

  - q: 3-2-1 规则和 4-2-1-1-0 规则的区别？
    a: 4-2-1-1-0 是 3-2-1 的延伸——多 1 份不可变备份、加上 0 个错误验证。本质仍是空间冗余、不解版本历史问题。
...
3-2-1 备份原则就是 3 份文件、2 种存储媒介、1 份异地。这条规则由摄影师 Peter Krogh 在 2005 年[《The DAM Book》](https://www.oreilly.com/library/view/the-dam-book/9780596008550/)（O'Reilly Media）订下，[CISA 至今仍以此为标准备份建议](https://www.cisa.gov/audiences/small-and-medium-businesses/secure-your-business/back-up-business-data)：
...
3-2-1 能防的是硬盘损毁、机房失火、勒索软件加密（[根据 Sophos 2024 年针对 14 国、5,000 位 IT 主管的调查，过去一年有 59% 的组织遭勒索软件攻击](https://www.sophos.com/en-us/blog/the-state-of-ransomware-2024)）——所有让文件「不见」的情况。它不防的是文件还在、但内容变错了——你自己覆盖掉版本、同事改错共享文件夹、想找 3 个月前那一版翻不到。

而后面这一类绝不是少数边缘案例。[Handy Recovery 2024 数据丢失调查](https://www.handyrecovery.com/data-loss-statistics/)显示，约四分之三的电脑用户承认自己曾误删过重要数据，而且误删是单一最常见的数据丢失原因——比硬件故障还高。这每一个时刻，3-2-1 都帮不上忙。
...
2026 年硬盘可靠（[Backblaze 2024 年硬盘报告显示，旗下逾 30 万颗硬盘的年化故障率为 1.57%，低于前一年的 1.70%](https://www.backblaze.com/blog/backblaze-drive-stats-for-2024/)）、云端实时同步，「3」变成了什么？**变成同一个错误被实时复制到 3 个位置**。
...
3-2-1 是 Peter Krogh 2005 年订下的备份规则——3 份文件、2 种存储媒介、1 份存放异地。设计目的是让任何单一硬件故障、媒介老化、机房灾难都无法让你的文件完全消失。但这是空间冗余——同一个错误的版本，3-2-1 会尽责复制到 3 个位置。

**Q2: 3-2-1 备份原则防什么、不防什么？**

3-2-1 能防硬盘损毁、机房失火、勒索软件加密。但它不防操作失误——你自己覆盖版本、同事改错共享文件夹、云端同步把错的版本传到三个位置，3-2-1 都救不了。要救这层需要版本历史工具（例如 Keeply）。

**Q3: 为什么做了 3-2-1 备份还是会丢档？**

3-2-1 的「3 份」是空间冗余、不是时间冗余。2005 年硬盘寿命短、媒介易坏，多放几份是对抗物理损毁；2026 年云端实时同步，「3」变成同一个错误被实时复制到 3 个位置。你需要的不只是多份备份，而是能回溯时间点的版本历史。

**Q4: 云端备份算 3-2-1 的「异地」吗？**

算。但 iCloud、OneDrive、Google Drive 是同步不是备份——你删除或覆盖会实时同步到云端、不防操作失误。异地要求只解决物理隔离问题，版本历史是另一层需求。详见 [Keeply 跟备份、云端工具有什么不一样](/zh-cn/post/what-keeply-saves-vs-backup-cloud/)。

**Q5: NAS 算 2 种媒介吗？**

NAS 加本机硬盘可算 2 种媒介。但 RAID 不算备份——RAID 防的是硬盘坏掉、不防你自己删错。

**Q6: 3-2-1 规则和 4-2-1-1-0 规则的区别？**

4-2-1-1-0 是 3-2-1 的延伸——多 1 份不可变备份、加上 0 个错误验证。本质仍是空间冗余、不解版本历史问题。

**Q7: 个人工作者也需要 3-2-1 备份原则吗？**

看文件重要性。判断标准只有一个——丢了会不会痛？跟个人或企业身份无关。会痛就需要。3-2-1 是必要但不足够的基础——还需要搭配版本历史才能应对操作失误场景。

**Q8: Keeply 已经是 3-2-1 吗？**

Keeply 内建 3-2-1-shape 位置层（本机工作副本 + 正本 + 备援位置），加上版本历史（重要时刻手动保存＋可选每 15/30/60 分自动），以及「发行版」冻结机制（把某次成果标成「给客户 v2.3 那版」、不被后续保存覆盖）。一套工具同时做空间冗余 + 时间冗余 + 发行版冻结。
