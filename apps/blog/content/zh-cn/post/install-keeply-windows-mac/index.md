...
faq_schema:
  ...
  - q: 安装完成后第一步该做什么？
    a: 打开 Keeply 点「新增项目」，选一个你**目前正在做且不想搞丢**的文件夹拖进去。初次扫描约 1-2 分钟建立基准版本，之后你在重要时刻点「保存版本」存一版（可写笔记）；想全自动就开后台自动保存，Keeply 每 15/30/60 分自动存一次。
  - q: 安装过程常见错误有哪些？
...
装完当天打开时间轴，会看到这样的历史——「首次保存 — 刚装好 Keeply」自己一行、带一个首次保护标记。

![Keeply 时间轴：从新增项目文件夹到首次保存 — 刚装好 Keeply，整条第一天的版本历史](timeline.svg)

那条「首次保存」是怎么来的？打开项目文件夹后，点 Keeply 主窗口的「**保存版本**」按钮，跳出这个对话框：

![Keeply 保存版本对话框：Q2-proposal.docx 1 个文件变更 + 笔记「首次保存 — 刚装好 Keeply」](save-dialog.svg)

写一行笔记告诉未来的自己「这版是装完当下的版本」，半年后回头翻时间轴看到的是描述、不是时间戳。
...
| 装完打不开 | 重启一次；仍不行寄 [keeply.work/contact](https://keeply.work/contact) |
