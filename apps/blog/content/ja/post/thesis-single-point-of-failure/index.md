...
faq_schema:
  ...
  - q: 修士論文のバージョン管理 4 ステップを具体的に教えてください。
    a: ステップ 1：1 日の終わりに日付入りでコピー保存（例：「論文-0423.docx」）。ステップ 2：指導教員へ送る前に「教員提出」と明示した別コピーを保管。ステップ 3：Keeply のようなツールで保存ごとに自動バージョン化＋メモ（「3.2 章を書き直し」）を残し、数か月後でもタイムラインから差分を見て復元。ステップ 4：少なくとも 1 部はこのノート PC 外に（クラウド・外付け SSD・USB のいずれか）。
  - q: このレイヤーが不要なのはどんな場合？
...
| ツール層の自動版本（[Keeply](https://keeply.work)） | 保存ごとに自動記録、日付横断の差分ビュー | ディスク全体の物理破損（バックアップと併用） | 対応する |
...
**3. ツールに毎バージョンを覚えさせる。** これがステップ 1 と 2 が届かない、あるいは続かない場所で、ツールが補う層だ。[Keeply](https://keeply.work) はそのために設計されている。保存のたびにシステムが自動的に 1 バージョンを残す。ファイルは今のフォルダにそのまま残り、引っ越しもツール乗り換えもいらない。**差分ビュー**で v5 と v6 のどの文字が変わったかを直接見られる。教員に聞かれたら、クリック 2 回で開ける。
