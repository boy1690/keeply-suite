...
![モック UI OneDrive 同期通知：緑チェック + 「全て最新の状態です」+ 黄色警告「同期済 ≠ あなたのデータが安全」](onedrive-sync-popup.svg)
...
![モック UI SharePoint バージョン履歴 list：v8 田中削除（紅）/ v7 鈴木 復元ボタン（青）/ v6 / v5 過去版](sharepoint-version-history.svg)
...
![モック UI Excel #REF! cell grid：B-D 欄全部 #REF! / A 欄クライアント名保留 / 公式列で 削除済 Sheet 参照](excel-ref-error-grid.svg)
