...
faq_schema:
  ...
  - q: Photoshop の自動保存はバージョン履歴と同じものですか？
    a: 違います。自動保存 (Auto Recover) が解くのはただ一つ、Photoshop がクラッシュしたり電源が落ちたりしたときに未保存の作業を失わないようにすることです。「30 分前の色のほうが好きだったから戻したい」という要望は対象外です。ファイル単位のバージョン履歴は別の層のツールで、大事な瞬間に（メモを添えて）バージョンを保存するか、自動保存をオンにして一定間隔で変更を取り込み、そのどれでも半年後に呼び戻せます。
  ...
  - q: デザイナーはファイル単位のバージョン履歴をどう補えばいい？
    a: Photoshop の外側にファイル単位のバージョン履歴の層を追加します。Keeply のようなツールなら、大事な瞬間に「クライアント承認版」などのメモを添えて PSD のバージョンを保存できますし、自動保存をオンにすれば一定間隔（15〜30 分ごと）でも変更を取り込みます。30 分前でも 6 か月前のバージョンでも、2 クリックで呼び戻せます。Photoshop の起動回数や開き方には依存しません。
  - q: このレイヤーが不要なケースは？
...
| **バージョン履歴** | 残す各バージョン（自分で保存、または一定間隔の自動保存） | 残した各バージョンの完全なスナップショットを永続保存 | ❌ |
...
欠けた層は Photoshop の外側に住んでいる——あなたがバージョンを残しておく別の場所、アプリケーションよりも一段上に位置する層だ。
...
残った空白を、Keeply が埋めようとしている。仕組みは単純：大事な瞬間に Keeply のウィンドウから（メモを添えて）バージョンを保存する。さらに自動保存をオンにすれば、Keeply が 15〜30 分ごとにフォルダをポーリングし、見つかった変更を保存する——どのバージョンもライブファイルとは別の場所に残るので、現在の作業には触れない。重い PSD（500MB クラスの単一ファイルでも）も背景で適切に処理される。Keeply は大容量ファイル向けの仕組みでディスクを膨らませない。
...
ついでに言うと：Keeply はあなたがすでに使っている Adobe Creative Cloud、Time Machine、各種のクラウド同期と並行して動く。どれも置き換えない。それらが対応していない一つの空白だけを埋める——バイナリのクリエイティブファイルのための、永続的なファイル単位バージョン履歴。あなたが保存したバージョンに、任意の自動保存を加えた層だ。
...
Keeply が残すのは、あなたが保存すると決めたバージョン（または一定間隔の自動保存）——「保存した後」の時間軸だ。「Cmd+S を押す前」のあの未保存の瞬間は、Photoshop 内蔵の自動保存が担当する。二つの仕組みがそれぞれ別の区間を、並行して受け持っている。
