...
OneDrive 個人版のごみ箱保存期間は 30 日、ビジネス版（SharePoint）は 93 日（[Microsoft 公式ドキュメント](https://learn.microsoft.com/ja-jp/sharepoint/retention-and-deletion)）。期間を超えると自動的に完全削除されます。

### 5. ゴミ箱の保存期間が過ぎた

Windows 10 / 11 の「ストレージ センサー」機能を有効にすると、ゴミ箱内のファイルが 30 日経過した時点で自動的に完全削除されます。[Microsoft 公式の説明](https://support.microsoft.com/ja-jp/windows/manage-drive-space-with-storage-sense-654f6ada-7bfc-45e5-966b-e24aded96ad5)によれば、この機能は初期状態ではオフですが、OEM の初期設定や過去の空き容量整理でオンになっている PC も少なくありません。

オンになっている場合、「1 ヶ月前に削除したファイル」が静かに消えていることになります。有効かどうかの確認・無効化は、設定アプリ → システム → 記憶域 → ストレージ センサーから行えます。
...
ファイルが OneDrive または SharePoint 同期フォルダにあった場合、OneDrive Web 版から「ごみ箱」を確認できます。OneDrive 個人版で 30 日以内、SharePoint で 93 日以内（[Microsoft 公式ドキュメント](https://learn.microsoft.com/ja-jp/sharepoint/retention-and-deletion)）なら、削除されたファイルが残っている可能性が高いです。
