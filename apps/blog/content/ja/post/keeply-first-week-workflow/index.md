...
howto_schema:
  name: Keeply 最初の 1 週間 7 日間検証観察日記
  totalTime: PT7D
  steps:
    - name: Day 1 ファイルを追加する
      text: Keeply をインストールしたら普段どおりに作業して最初のファイルを保存します。昼休みの前に Keeply を開き、追加したファイルが自動でインターフェースに表示されタイムスタンプが付いていることを確認します。手動で追加する必要はありません。
      url: '#day-1'
    - name: Day 3 ファイルを編集する
      text: 前日のファイルを編集して複数回保存します。夕方にバージョンパネルを開き、17 個の細かい記録ではなくタイムスタンプ付きの 2〜4 個の重要なバージョンが表示されること、各バージョンをクリックして復元できることを確認します。
      url: '#day-3'
    - name: Day 5 削除テスト
      text: 重要でないテストファイルを意図的に削除します。Keeply の「削除済みファイル」一覧を開き、そのファイルがまだ存在していて「復元」をクリックして取り戻せることを確認します。これによりKeeplyの削除追跡がシステムのごみ箱と独立していることが検証できます。
      url: '#day-5'
    - name: Day 7 総合評価
      text: 今週の 3 点を振り返ります。Keeply が追加したファイルを認識したか、妥当な数のバージョンを残したか、削除後に取り戻せたか。3 つすべてが「はい」なら使い続けます。1 つでも合わなければそのまま削除してください。
      url: '#day-7'
---
...
![Keeply の版保存サイドパネル：proposal_acme.docx を追加 + メモ「Acme RFP 提案 — 初版ドラフト」+ 横幅いっぱいの青い保存ボタン](save-dialog.svg)
...
![Keeply のファイル版パネル：proposal_acme.docx が Day 1 の初版から今日のクライアントサインオフまで 5 つの版を蓄積、手書きメモ付き 2 版（最終価格／SLA 条項修正）と自動保存 3 版を含む](file-history.svg)
...
![Keeply の削除済みファイルパネル：Day 5 にたった今削除した test_doc_to_delete.docx + 数日前に削除した old_estimate.xlsx と moodboard_v1.png + 各エントリ横の復元リンク](deleted-files-panel.svg)
...
![Keeply 第 1 週のタイムライン：Day 1 インストール → proposal_acme.docx を追加 → Day 2 最終価格 → Day 5 test_doc 復元 + SLA 修正 → Day 7 クライアントサインオフ](timeline.svg)
