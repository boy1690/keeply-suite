...
faq_schema:
  ...
  - q: Microsoft AutoSave が説明していない制限は？
    a: 回避できない 4 つの制限があります：デスクトップ AutoSave は 1-2 バージョンしか戻れない；OneDrive のバージョン履歴は最大 500 バージョンで古いものは時間とともに間引かれる（無制限ではない）；ローカルファイルにはバージョン記録が一切ない；そしてセルレベルの差分がない。これらは技術的な限界ではなく、Microsoft の意図的なエンジニアリング選択です。
...
意外と知られていない：あなたの普段の働き方は **4 つすべて当てはまらない** 可能性が高い。デスクトップ保存（ローカルファイルには AutoSave がそもそも無い。[AutoSave は OneDrive／SharePoint 上のファイルだけの機能で、クラウドでは既定でオン](https://support.microsoft.com/ja-jp/office/what-is-autosave-6d6bd723-ebfd-4e40-b5f6-ae6e8088f7a5)）、個人版、デスクトップと web を行き来。だからグレーアウトはデフォルト状態で、あなたが何か間違ったわけじゃない。
...
| 2 | **OneDrive/SharePoint はバージョン数に上限・古い版は間引き** | 既定で最大 [500 バージョン](https://learn.microsoft.com/ja-jp/sharepoint/document-library-version-history-limits)、古いものは間引かれ上限到達で最古が削除——無制限ではない |
...
ファイルがどこに保存されていても、ツールが前のバージョンを保存しておく。**例**：macOS Time Machine（システム層、ディスク全体）、Keeply（ファイル層、指定した作業フォルダだけ）。**Keeply の違い**：あなたが保存したバージョンを——手動でメモを添えて、または任意の自動保存で 15〜30 分ごとに——完全保存、バージョン数の上限なしで残し、[OneDrive の 500 バージョン上限](https://learn.microsoft.com/ja-jp/sharepoint/document-library-version-history-limits)のように古い版が間引かれることもない。**制限 #1 + #2 + #3 を解決。**
