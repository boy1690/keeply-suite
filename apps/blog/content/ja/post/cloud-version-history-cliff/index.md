...
faq_schema:
  ...
  - q: Keeply はクラウドストレージの代替ですか?
    a: いいえ。Keeply はクラウド同期の上に重ねる別レイヤー——あなたが保存したバージョンをローカルに残し、時間 cap も計数 cap もありません。クラウドが同期とオフサイトコピーを担当、Keeply がバージョン履歴を担当。移行ではなくレイヤーを追加するだけです。
---
...
リフレームしましょう:クラウドを乗り換えなくてもこの問題は解けます。あなたのクラウドは同期には問題ありません。足りないのは**別レイヤー**——ファイル層レベルのバージョン履歴、時間 上限 なし——あなたが保存するバージョンを残します（手動で、または任意の自動保存 15〜30 分ごと）。

具体的に:

- **クラウド(4 社のうちどれでも)** は同期 + オフサイトコピーを担当
- **バージョン履歴レイヤー(Keeply または同類)** はあなたが保存するバージョン（手動 ＋ 任意の自動保存）を残し、時間 上限 なし、計数 上限 なし、保存時に「Keep forever」を決める必要なし
...
Keeply はこのレイヤーのリファレンス実装:あなたが保存したバージョンをローカルに保持、時間 上限 なし、計数 上限 なし、加えて「Release」凍結機構——あるバージョンを「これがクライアントに送った版」とマークすると、その スナップショット は後で 50 回保存しても永遠に残ります。2 か月前のバージョンの回復は約 2 クリック。
