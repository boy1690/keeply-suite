...
それからもう 1 つ、彼女が復元したのは特定のバージョンではなく、ファイルそのものでした。もし Sarah が 3 週間前の火曜日の `proposal_v3` ——今日のバージョンではなく——を欲しかったとしたら、必要なのは[バージョン履歴](/ja/post/file-version-management-complete-guide/)のレイヤーで、これは削除履歴とはまったく別の枝です。復元というのは、削除された瞬間の姿でファイルが戻ってくることを意味します。昨日のうちに行った 3 回の保存は、その中に焼き込まれてしまっています。
