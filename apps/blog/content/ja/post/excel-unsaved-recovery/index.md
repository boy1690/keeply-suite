...
- **ファイルが OneDrive や SharePoint にあり、自動保存がオンのとき。** これでかなりの範囲がカバーされます ── クラウドのバージョン履歴が、編集中の上書きの多くを拾ってくれます。ただし、できないことも知っておいてください。対象は同期されたコピーに限られ、[保存される履歴には上限があり](https://learn.microsoft.com/ja-jp/sharepoint/document-library-version-history-limits)、[自動保存はその場で（断りなく）上書きしていきます](https://support.microsoft.com/ja-jp/office/what-is-autosave-6d6bd723-ebfd-4e40-b5f6-ae6e8088f7a5)。その制限を読んだうえで自分には刺さらないと思えるなら、もう一層は要りません。
...
[自動保存](https://support.microsoft.com/ja-jp/office/what-is-autosave-6d6bd723-ebfd-4e40-b5f6-ae6e8088f7a5)（OneDrive/SharePoint）はクラウド保存のファイルには役立ちますが、その場で上書きしていく仕組みで、[バージョン履歴にも独自の上限があります](https://learn.microsoft.com/ja-jp/sharepoint/document-library-version-history-limits)。ローカルに置いているファイルは対象外で、ファイルを一覧して選べる「残るバージョンのタイムライン」とは別物です。
