...
Dropbox は「後から保存した版が勝ち、前の版を別保存する」設計（[Dropbox 公式の説明](https://help.dropbox.com/organize/conflicted-copy)）：二人同時編集、後からアップロードした版が勝ち、前の版は `(競合コピー)` として残る。
...
[Dropbox Help Center](https://help.dropbox.com/organize/conflicted-copy) が教える 対処：「2 つのファイルを開く、差分を比較、メインに手動統合、競合コピーを削除。」一聴合理的。
