...
![削除タイムマシン：左側に各時点で何を消したかを日付ごとに表示、右側のタイムマシン式ファイルツリーで消えたファイルを示し、チェックしてワンクリックで復元](deletion-time-machine.png)
...
![ファイルを一回クリックすると、メインタイムラインがそのファイルのバージョン履歴だけを表示](file-timeline.png)
...
![チーム機能の先行紹介：チームの動きを示す路線図。本線がメインのバージョン、各メンバーの作業コピーが支線になり、統合と審査の状態を表示（機能はまだ公開していません）](team-metro-map.png)
