...
faq_schema:
  - q: Word 내장 버전 기록은 무엇을 할 수 있나요?
    a: "Word 에는 세 가지 메커니즘이 있습니다: 자동 복구(크래시 구조용, 닫으면 지워짐), 자동 저장(입력하는 동안 클라우드에 저장), OneDrive 버전 기록(약 500 개 버전 스냅샷 보관). 셋 다 단기 저장 사고 구조용이라, 3 개월 전에 보낸 납품 버전 추적은 설계 목표에 없습니다."
  - q: 자동 복구、OneDrive、Time Machine 은 각각 얼마나 보관하나요?
    a: "자동 복구 는 파일을 정상적으로 닫으면 바로 지워지고; OneDrive 버전 기록은 기본 약 500 개 버전이며 초과하면 가장 오래된 것부터 자동 삭제하고; Mac Time Machine 은 매시간 스냅샷을 24 시간, 매일 스냅샷을 30 일 보관합니다. 메커니즘마다 보존 상한이 있어 3 개월이라는 선을 넘지 못합니다."
