---
title: "【2026 파일 관리】PPT는 버전을 저장해도, 3개월 후의 기억은 저장하지 못한다"
description: "Word 자동 복구, OneDrive 버전 기록, Time Machine 은 모두 저장 계층 구조 도구입니다 — 보존기간은 닫으면 사라지는 것부터 약 500 개 버전이 상한까지. 3 개월 전 납품한 그 버전을 되찾으려면 툴 계층의 상시 버전 기록과 납품 시점의 메타데이터가 필요합니다."
...
faq_schema:
  - q: Word 내장 버전 기록은 무엇을 할 수 있나요?
    a: "Word 에는 세 가지 메커니즘이 있습니다: AutoRecover(크래시 구조용, 닫으면 지워짐), 자동 저장(입력하는 동안 클라우드에 저장), OneDrive 버전 기록(약 500 개 버전 스냅샷 보관). 셋 다 단기 저장 사고 구조용이라, 3 개월 전에 보낸 납품 버전 추적은 설계 목표에 없습니다."
  - q: AutoRecover、OneDrive、Time Machine 은 각각 얼마나 보관하나요?
    a: "AutoRecover 는 파일을 정상적으로 닫으면 바로 지워지고; OneDrive 버전 기록은 기본 약 500 개 버전이며 초과하면 가장 오래된 것부터 자동 삭제하고; Mac Time Machine 은 매시간 스냅샷을 24 시간, 매일 스냅샷을 30 일 보관합니다. 메커니즘마다 보존 상한이 있어 3 개월이라는 선을 넘지 못합니다."
  - q: Word 버전 기록은 왜 3 개월 후까지 못 버티나요?
    a: "내장 버전 기록은 「저장 층」에 살며, 가장 최근의 실패한 쓰기에 대비하도록 설계됐고, 보존 기간은 평균 사용자가 한 달 안에 얼마나 자주 되돌아보는지에 맞춰져 있습니다. 3 개월 이상은 설계 목표가 아니라, 정리되는 게 합리적인 동작입니다. 이를 해결하려면 도구 층의 상시 버전 기록이 필요합니다."
  - q: 3 개월 전 납품 버전을 되찾으려면 무엇이 필요한가요?
    a: "두 개의 층이 필요합니다: 상시 버전 기록(당신이 저장한 버전이 Word 나 OneDrive 의 보존 정책과 무관하게 남는 것); 그리고 납품 메모 메타데이터(누가, 언제, 어느 버전인지 내보낼 때 자동으로 새겨지는 것). Keeply 는 이 두 가지를 모두 제공합니다."
  - q: Google Docs 의 수정 버전은 얼마나 보관되나요?
    a: "Google 은 명확한 보존 기간을 공개하지 않습니다. 공식 문서는 공간 절약을 위해 오래된 수정 버전이 병합될 수 있다고 밝히며, 실제로 3 개월 이상 된 수정 버전은 자동으로 병합되거나 정리되는 경우가 많아 장기 납품 버전 추적에 믿고 쓸 수 없습니다."
---
...
2. 자동 복구 / OneDrive / Time Machine: 각각 얼마나 보관되나?
...
- **OneDrive 버전 기록**: 매 저장마다 스냅샷을 남겨 임의 시점으로 돌아갈 수 있음. Microsoft 의 [SharePoint 버전 관리 문서](https://learn.microsoft.com/ko-kr/sharepoint/document-library-version-history-limits) 에 따르면 기본 500 개 주 버전 보관 (개인 Microsoft 계정은 25 버전).
...
| OneDrive 버전 기록 | 기본 약 [500 개 버전](https://learn.microsoft.com/ko-kr/sharepoint/document-library-version-history-limits) (개인 계정은 25 버전) | 500 초과시 가장 오래된 것 정리 | 단기 롤백 |
...
![Keeply 버전 비교 뷰: 4/12 클라이언트 확정 버전 vs 5/4 가격 수정 버전 + proposal.docx 32 줄 차이 + Basic $99→$149 / Pro $299→$399](diff-viewer.svg)
