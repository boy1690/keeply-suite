...
faq_schema:
  - q: Keeply 는 무엇을 보관하나요? 백업이나 클라우드와 뭐가 다른가요?
    a: "Keeply 는 「당신 자신의 수정 이력」을 보관합니다: 중요한 순간에 메모와 함께 버전을 저장하거나, 선택적 자동 저장을 켜 15-30 분마다 작업을 담아 두면 됩니다 — 어느 것을 남길지 일일이 고민할 필요가 없습니다. 「잘못 고쳐서 되돌리고 싶다」는 상황을 해결하는데, 이는 백업도 클라우드도 다루지 않는 층입니다."
  - q: 백업 도구는 무엇을 보관하고, 어떤 상황에서 필요한가요?
    a: 백업 도구는 「특정 시점의 디스크 전체 완전 스냅샷」을 보관해, 디스크 고장, 노트북 분실, 서버실 화재 같은 재난 상황에 대비합니다. Time Machine、3-2-1 이 여기에 속합니다. 하드웨어를 구하지, 당신이 잘못 저장한 것을 구하지는 못합니다.
  - q: 클라우드 도구는 무엇을 보관하고, 무엇을 해결하나요?
    a: 클라우드 도구는 「여러 기기 간 최신 버전 동기화」를 보관해, 휴대폰、태블릿、노트북이 같은 파일을 봐야 하는 상황을 해결합니다. Dropbox、OneDrive、iCloud 가 여기에 속합니다. 기기 전환을 구하지, 수정 이력을 구하지는 못합니다.
  - q: 도구가 도대체 몇 개나 있어야 충분한가요?
    a: 무엇을 두려워하느냐에 달려 있습니다 — 디스크 고장이 두려우면 백업이, 기기 간 단절이 두려우면 클라우드가, 자신의 잘못된 수정이 두려우면 Keeply 가 필요합니다. 셋은 서로 다른 층의 도구라 서로 대체하지 않습니다. 가장 흔한 「잘못 고쳤다」 상황은 바로 앞의 두 가지가 못 구하는 경우입니다.
---
...
버전 이력 기능도 있긴 해요. 하지만 다들 상한이 있어요 — [Dropbox 표준 요금제와 Google Drive 는 보통 30일](https://help.dropbox.com/delete-restore/version-history-overview), [OneDrive／SharePoint 는 버전 수 상한(기본 500)](https://learn.microsoft.com/ko-kr/sharepoint/document-library-version-history-limits)이라 오래된 버전은 솎여요. 그게 지나면 사라져요.
...
![Keeply 파일 이력 패널 proposal.docx: 12주 전부터 오늘까지 6 개 버전 + 각 버전 메모 + 로컬 보존 표시](file-history.svg)
...
![Keeply 타임라인 proposal.docx: 수동 메모가 있는 2 개 버전을 포함한 4 개 버전](timeline.svg)
