...
faq_schema:
  ...
  - q: "Keeply는 한컴오피스와 충돌하나요?"
    a: "충돌하지 않습니다. 중요한 순간에 「버전 저장」을 누르면 Keeply가 자체 타임라인에 한 버전을 남기고, 자동 저장을 켜 두면 작업 폴더를 15~30분마다 살펴 바뀐 부분을 담아 둡니다 — HWP의 자동 저장 / .bak과 병행. HWP는 비정상 종료 / 직전 버전을 해결하고, Keeply는 무제한 타임라인 + 중요 버전 잠금을 보완합니다."
...
- **외부 도구 타임라인**: 중요한 순간에 「버전 저장」을 눌러 **자체 타임라인**에 한 버전을 남기거나, 자동 저장을 켜 두면 Keeply가 작업 폴더를 15~30분마다 살펴 바뀐 부분을 담아 둠 — 한글 내장의 제한된 `~AutoSave/`에 의존하지 않음, 반년 전 버전 기록까지 보관(1-2 버전 아님)
