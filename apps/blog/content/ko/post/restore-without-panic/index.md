...
대부분의 경우, OS에는 복구할 흔적이 전혀 남아 있지 않다. 게다가 이건 드물게 일어나는 사고가 아니다 — [Handy Recovery의 2024년 조사에서, 실수로 인한 삭제는 데이터 손실의 가장 흔한 단일 원인으로, 하드웨어 고장보다도 앞섰다](https://www.handyrecovery.com/data-loss-statistics/).
...
현대 컴퓨터의 대부분은 SSD(솔리드 스테이트 드라이브)를 쓰고 — [2024년에는 노트북 SSD 탑재율이 약 100%에 달해, 이제 사실상 모든 신형 노트북에 SSD가 들어간다는 뜻이다(TrendForce)](https://www.trendforce.com/presscenter/news/20251107-12774.html) — Windows 7 이후로는 TRIM이 기본으로 활성화되어 있다. 당신이 파일을 삭제하면, OS는 즉시 TRIM 명령을 보내 그 블록을 "재사용 가능"으로 SSD에 알린다.
