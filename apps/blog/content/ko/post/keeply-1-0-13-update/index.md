...
![삭제 타임머신: 왼쪽에는 날짜별로 각 버전에서 무엇을 지웠는지 나열되고, 오른쪽 타임머신 파일 트리에는 지워진 파일이 표시되며, 선택하면 한 번에 복원합니다 (화면은 중국어)](deletion-time-machine.png)
...
![파일 하나를 누르면 메인 타임라인이 자동으로 그 파일의 버전 이력만 보여 줍니다 (화면은 중국어)](file-timeline.png)
...
![팀 협업 미리보기: 팀 활동 노선도. 본줄기는 메인 버전, 팀원별 작업 사본은 지선이며, 병합과 검토 상태가 표시됩니다 (기능은 아직 미공개, 화면은 중국어)](team-metro-map.png)
