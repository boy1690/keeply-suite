...
faq_schema:
  ...
  - q: Photoshop 자동 저장과 버전 이력은 같은 건가요?
    a: 다릅니다. 자동 저장(Auto Recover)이 해결하는 문제는 단 하나, Photoshop이 크래시하거나 전원이 꺼졌을 때 저장되지 않은 작업을 잃지 않도록 하는 것입니다. 「30분 전의 색감이 더 좋았으니 되돌려달라」는 요구는 대상이 아닙니다. 파일 단위 버전 이력은 별도의 층입니다. 중요한 순간에 메모와 함께 버전을 저장하거나, 자동 저장을 켜 일정 간격마다 변경을 담아 두면, 몇 달 뒤에도 그 어느 버전이든 불러올 수 있습니다.
  ...
  - q: 디자이너는 파일 단위 버전 이력을 어떻게 보완하나요?
    a: Photoshop 바깥에 파일 단위 버전 이력 층을 추가합니다. Keeply 같은 도구는 중요한 순간에 「클라이언트 승인본」 같은 메모와 함께 PSD 버전을 저장하게 해 줍니다 — 그리고 자동 저장을 켜 두면 15~30분마다 변경도 함께 담아 둡니다. 30분 전이든 6개월 전이든 두 번 클릭으로 불러올 수 있고, Photoshop을 어떻게 또는 몇 번 열었는지에 의존하지 않습니다.
...
| **버전 기록** | 당신이 남긴 버전마다 (직접 저장하거나, 자동 저장이 일정 간격으로) | 남긴 각 버전의 완전한 스냅샷, 영구 보존 | ❌ |
...
빠진 층은 Photoshop 바깥에 산다 — 네가 버전을 남겨 두는 별도의 자리, 애플리케이션보다 한 단계 위의 층이다.
...
남은 빈자리를 Keeply가 메우려 한다. 로직은 단순하다: 중요한 순간에 Keeply 창에서 메모와 함께 버전을 저장하면(또는 자동 저장을 켜 두면 Keeply가 15~30분마다 폴더를 살펴 바뀐 부분을 담아 둔다), Keeply가 그 순간의 완전한 버전을 한 부 보존한다 — 원본과 분리해서, 현재 작업에는 손대지 않고. 무거운 PSD (500MB 한 장 같은 것)도 백그라운드에서 우아하게 처리된다 — Keeply는 대용량 파일용 메커니즘을 써서 디스크가 부풀지 않게 한다.
