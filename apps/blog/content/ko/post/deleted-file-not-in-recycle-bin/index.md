...
OneDrive 개인 버전의 휴지통 보존 기간은 30일, 비즈니스 버전(SharePoint)은 93일. 기간을 넘기면 자동으로 영구 삭제됩니다.

### 5. 휴지통의 보존 기간이 지났다

최근의 Windows 10 / 11에서는 '저장 공간 센스' 기능이 기본으로 활성화되어 있고, 휴지통 안의 파일이 30일 경과하면 자동으로 영구 삭제되는 설정이 초기값입니다.

이 기능은 설정 앱 → 시스템 → 저장 공간 → 저장 공간 센스에서 비활성화할 수 있지만, 초기 설정 그대로의 PC에서는 '1개월 전에 삭제한 파일'이 조용히 사라지고 있다는 뜻입니다.
...
파일이 OneDrive 또는 SharePoint 동기화 폴더에 있었던 경우, OneDrive 웹 버전에서 '휴지통'을 확인할 수 있습니다. OneDrive 개인 버전에서 30일 이내, SharePoint에서 93일 이내라면 삭제된 파일이 남아 있을 가능성이 높습니다.
