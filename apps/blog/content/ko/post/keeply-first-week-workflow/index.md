...
howto_schema:
  name: Keeply 첫째 주 7일 검증 관찰 일지
  totalTime: PT7D
  steps:
    - name: Day 1 파일 추가
      text: Keeply 설치 후 평소대로 일하며 첫 번째 파일을 저장합니다. 점심 전에 Keeply를 열어 새로 추가한 파일이 인터페이스에 타임스탬프와 함께 자동으로 나타나는지 확인합니다. 수동으로 추가할 필요는 없습니다.
      url: '#day-1'
    - name: Day 3 파일 수정
      text: 어제 작업한 파일을 수정하고 여러 번 저장합니다. 저녁에 버전 패널을 열어, 사소한 기록 17개가 아닌 타임스탬프가 있는 핵심 버전 2~4개가 표시되는지, 그리고 각 버전을 클릭해 복원할 수 있는지 확인합니다.
      url: '#day-3'
    - name: Day 5 삭제 테스트
      text: 중요하지 않은 테스트 파일을 일부러 삭제합니다. Keeply의 「삭제된 파일」 목록을 열어 해당 파일이 여전히 존재하고 「복원」을 클릭해 되살릴 수 있는지 확인합니다. 삭제가 시스템 휴지통과 독립적으로 처리되는지 검증하세요.
      url: '#day-5'
    - name: Day 7 종합 평가
      text: 이번 주 세 가지를 돌아봅니다. Keeply가 새로 추가한 파일을 확인했는지, 적당한 수의 버전을 남겼는지, 삭제 후 찾아올 수 있는지. 세 가지 답이 모두 「예」라면 계속 사용하고, 하나라도 맞지 않으면 바로 제거합니다.
      url: '#day-7'
---
