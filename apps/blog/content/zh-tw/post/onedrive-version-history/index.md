...
description: "Microsoft Learn 自己寫得很清楚：OneDrive 每個檔案最多保留 500 個 主要版本、資源回收筒 個人 30 天 / 公司 93 天、自動回復 是另一回事。本文拆三件機制 + 用 Keeply 發行版凍結 接住超過 上限 後的場景。"
...
image_alt_data: "OneDrive 版本歷史 500 上限 + 30 天 資源回收筒 + 自動回復 三機制示意圖，Microsoft Learn 官方文件 URL 引用"
faq_schema:
  - q: OneDrive 版本歷史最多保留幾個版本？
    a: "根據 Microsoft Learn 官方文件，OneDrive 每個檔案最多保留 500 個 主要版本。超過第 500 次儲存後，最舊的版本會被自動刪掉、不會通知你。資源回收筒 保留期是另一回事（個人 30 天 / 公司 93 天）、只適用於刪除檔案、不是修改檔案。"
  - q: OneDrive 版本歷史可以保留多久？
    a: "Version history 本身沒有時間限制（受 500 major-version 上限 限制）。有時間限制的是 資源回收筒——個人帳號 30 天、公司或學校帳號 93 天。這兩個系統常被誤認為是同一個。"
  - q: OneDrive 版本歷史和 AutoRecover 一樣嗎？
    a: "不一樣。Version history 是 OneDrive 雲端的每一版檔案。自動回復 是 Office 桌面客戶端的緊急救援（預設 10 分鐘一次）、只在軟體當機或強制關閉前沒儲存時有用。兩者完全不同存儲層。"
  - q: 為什麼 OneDrive 找不到我 6 個月前的版本？
    a: "兩個可能原因。(a) 你超過 500 versions 上限、Microsoft 自動刪了最舊的；(b) 你搜的是 資源回收筒 不是 version history、而 30 天的窗已經過了。重度使用者（每天 ≥3 次儲存）大約 7-8 個月就會撞到 上限。"
  - q: 超過 500 個版本後怎麼辦？
    a: "OneDrive 會默默刪除最舊的版本來騰出空間給新的——沒警告、沒遷移到 archive、沒選項保留舊版。你可以手動 pin 一個版本、但仍在 500 池內。要解決這個 上限，需要 發行版凍結 機制無上限的工具（例如 Keeply）。"
  - q: Keeply 跟 OneDrive 衝突嗎？
    a: "不衝突。Keeply 跟 OneDrive 並行運作——它監看你的工作資料夾、維護自己的時間軸、不受 OneDrive 500 上限 限制。OneDrive 留給同步協作、Keeply 給你 無上限 的單檔版本歷史 + 筆記 + 發行版凍結。"
---
...
這不是 錯誤、不是 BUG report。是 [Microsoft Learn 文件](https://learn.microsoft.com/en-us/sharepoint/document-library-version-history-limits)早就寫清楚的 500 主要版本 上限。但 90% 的 OneDrive 版本歷史教學文章只教**怎麼用**、不講**它什麼時候會碰壁**。本文補這層——三件被混在一起的 OneDrive 機制（version history 500 上限 / 資源回收筒 30 天 / 自動回復）拆開、加上 [Keeply](https://keeply.work) 怎麼接住超過 上限 後的場景。
...
「Client signed v2.3 — 董事會核可」自己一行、有 Release tag——是她今天客戶確認後、主動點 Keeply 主視窗「儲存版本」+ 寫筆記存的：

![Keeply 儲存版本對話框：proposal.docx 筆記「Client signed v2.3」](save-dialog.svg)

寫一行「Client signed v2.3 — 董事會核可」、儲存版本。明年 3 月翻時間軸看 tag 就有——不受 OneDrive 500 上限 影響、不被自動刪除。
...
[Microsoft Learn 文件](https://learn.microsoft.com/en-us/sharepoint/document-library-version-history-limits)寫得清楚：SharePoint / OneDrive 文件庫每個檔案最多保留 **500 個 主要版本**（搭配主要 / 次要 版本管理 開啟時、可再加 511 個 次要版本）。
...
相關詳細請看 [Photoshop 自動儲存 不是 version history](/zh-tw/post/photoshop-autosave-not-version-history/)——Adobe 那層同款混淆機制。
...
客戶來信當下、Tina 在 Keeply 時間軸找到 2 月 14 日那筆「Client signed v2.3」、按右鍵還原——跳出對話框：

![Keeply 還原檔案對話框：要把 proposal.docx 還原到「客戶簽約 v2.3 — 5/4」嗎？還原前自動建立目前版本快照、可隨時 Undo](restore-file-dialog.svg)

她按「還原」。3 秒鐘 `proposal.docx` 回到 2 月那版的狀態；目前版本被自動快照、想反悔隨時 Undo。OneDrive 留給同步協作（這是它強項）、Keeply 給她 無上限 的單檔版本歷史。
...
不衝突、並行運作。OneDrive 同步協作、Keeply 給 無上限 單檔版本歷史 + 筆記 + 發行版凍結。
