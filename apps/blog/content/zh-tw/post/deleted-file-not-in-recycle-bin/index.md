...
但在動手之前，有 4 件事絕對不能做。請先看下面 5 個原因找出你的狀況，再進到救援方法。順序很重要。（如果不想一條一條讀，可以先用免費的[「我的檔案救得回來嗎？」三題自我檢測](https://keeply.work/zh-TW/tools/can-i-recover-my-file)快速判斷一下機率。）
...
OneDrive 個人版的雲端垃圾桶保留 30 天，商用版（SharePoint）保留 93 天（[Microsoft 官方文件](https://learn.microsoft.com/zh-tw/sharepoint/retention-and-deletion)）。過期就會被自動永久刪除。

### 5. 垃圾桶保留期限到了

Windows 10 / 11 有個「儲存空間感知」（Storage Sense）功能：開啟後，會把垃圾桶裡超過 30 天的檔案自動永久刪除。根據 [Microsoft 官方說明](https://support.microsoft.com/zh-tw/windows/manage-drive-space-with-storage-sense-654f6ada-7bfc-45e5-966b-e24aded96ad5)，這個功能預設其實是「關閉」的——但不少 PC 被手動開過（OEM 預裝、或之前清理空間時順手開），於是「一個月前刪除的檔案」就被悄悄清掉了。

要確認自己有沒有開、或想關掉，到「設定」→「系統」→「儲存空間」→「儲存空間感知」就能查。
...
如果檔案原本是在 OneDrive 或 SharePoint 同步資料夾，可以從 OneDrive 網頁版去看「資源回收筒」。OneDrive 個人版 30 天內、SharePoint 93 天內（[Microsoft 官方文件](https://learn.microsoft.com/zh-tw/sharepoint/retention-and-deletion)），被刪檔案很可能還在。
