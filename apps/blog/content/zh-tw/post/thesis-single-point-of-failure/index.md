...
howto_schema:
  ...
  steps:
    ...
    - name: 交教授前獨立留檔
      ...
      url: '#thesis-4-step'
    - name: 工具自動記錄每版差異
      text: 導入 Keeply 讓你存的每一版自動留紀錄、可以寫筆記（例如「3.2 節重寫」「教授第三輪回饋後」），半年後直接看時間軸點開差異視圖，不用手動翻 v1 v2 v3 檔名。
      url: '#thesis-4-step'
    - name: 至少一份不在這台筆電
...
| **[Keeply](https://keeply.work)** | **你存的每一版自動記、可以寫筆記、半年後翻時間軸看得到差異** | **整顆 SSD 物理壞掉（要搭 3-2-1）** |
