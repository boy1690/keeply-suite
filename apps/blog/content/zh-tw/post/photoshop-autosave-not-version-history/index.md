...
faq_schema:
  ...
  - q: Photoshop 自動儲存跟版本歷史一樣嗎？
    a: 不一樣。自動儲存 (Auto Recover) 只解一個問題：Photoshop 當機或斷電時讓你不丟掉現在還沒存的進度。它不負責「半小時前的那版顏色我比較喜歡，幫我還原」。檔案級版本歷史是另一層工具——在背景每隔一段時間（你可設 15/30/60 分）自動把變更另存一版、半年後還叫得回來。
  - q: Photoshop 歷史紀錄面板能還原昨天的版本嗎？
    a: 不行。歷史紀錄面板只記錄這個檔案這次開啟後的操作步驟，一旦你關閉 PSD 再打開，整段歷史就清空。它解的是「我剛剛這 50 步要回去某一步」，不是「昨天那版我要拿回來」。
  - q: 設計師怎麼補齊檔案級版本歷史？
    a: 在 Photoshop 外面加一層自動版本記錄。像 Keeply 這類工具會在背景每隔一段時間（你可設 15/30/60 分）把改動過的 PSD 自動另存一份進時間軸，你也能在重要時刻手動存一版並附筆記（如「客戶確認版」），半小時後想對比、半年後想還原都點兩下就好，跟你怎麼開 Photoshop、開了幾次都無關。
  - q: 什麼時候設計師不需要再裝這層工具？
...
想自己去資料夾翻一遍確認？實際路徑在：Mac `~/Library/Application Support/Adobe/Adobe Photoshop {version}/AutoRecover/`、Windows `%AppData%/Adobe/Adobe Photoshop {version}/AutoRecover/`（資料夾名是英文 `AutoRecover`）。前幾次 session 的舊 `.psb` 可能還躺著、但今天的工作從來沒被寫進去、也就還原不出來。
