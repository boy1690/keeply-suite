...
|---|---|---|
| **File History** | 把你選的資料夾[按小時複製到外接硬碟](https://support.microsoft.com/zh-tw/windows/backup-and-restore-with-file-history-7bf065bf-f1ea-0a78-c1cf-7dcf51cc8bfc) | 每版只有時間戳、不寫筆記。而且外接硬碟沒接就不存 |
| **Windows Backup**（備份與還原） | 把整台電腦拍一張快照 | 給你整台電腦、不是單一檔案的版本史 |
| **版本歷史**（OneDrive） | 雲端記每個檔案改了幾次 | 純時間戳、[保留期過了就消失](https://learn.microsoft.com/zh-tw/sharepoint/document-library-version-history-limits)、只對有同步的檔案 |
...
| 兩週前不小心刪了檔案 | ✅ 在監看的資料夾裡的話 | ✅ 映像有 | ✅ [30 天回收筒](https://learn.microsoft.com/zh-tw/sharepoint/retention-and-deletion) | ✅ 版本歷史裡有 |
