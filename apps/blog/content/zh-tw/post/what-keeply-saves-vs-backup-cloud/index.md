...
faq_schema:
  ...
  - q: OneDrive / Dropbox 不是也有版本歷史嗎？
    a: 有、但都有上限——Dropbox 標準方案、Google Drive 通常 30 天，OneDrive／SharePoint 則是版本數上限（500 版），而且每一版只有時間戳、沒有「為什麼存」的筆記。3 個月後想找回「業主確認版」那一版，雲端通常已經沒了——就算沒被刪，也只有 14:00、15:00 一排時間戳要你自己猜。
...
它們也有版本歷史，但都有上限——[Dropbox 標準方案、Google Drive 通常 30 天](https://help.dropbox.com/delete-restore/version-history-overview)就刪，[OneDrive／SharePoint 則是版本數上限（預設 500 版）](https://learn.microsoft.com/zh-tw/sharepoint/document-library-version-history-limits)、舊版會被稀釋。
...
跨檔案的歷史也是同一條邏輯。同一份 `proposal.docx` 從 12 週前的初稿到今天的最新版，Keeply 全留下來——而且是過了雲端 30 天天花板之後還在的本機層：

![Keeply 檔案歷史面板 proposal.docx：12 週前到今天的 6 個版本 + 每版筆記 + 本機保留標示](file-history.svg)

3 個月前那份你想對照的合約底稿——Dropbox 已經過保留期、Time Machine 還在但要先還原才挑得到——Keeply 在這條清單上直接點那一行就回去。
