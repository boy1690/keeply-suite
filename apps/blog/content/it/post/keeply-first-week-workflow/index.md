...
howto_schema:
  name: Keeply, prima settimana — diario di osservazione di 7 giorni
  totalTime: PT7D
  steps:
    - name: Giorno 1 — aggiungi un file
      text: Dopo aver installato Keeply lavora normalmente e salva il primo file; prima di pranzo apri Keeply e verifica che il file aggiunto compaia automaticamente nell'interfaccia con un timestamp, senza doverlo aggiungere a mano.
      url: '#day-1'
    - name: Giorno 3 — modifica un file
      text: Modifica il file di ieri e salvalo più volte; verso sera apri il pannello versioni e verifica che mostri 2-4 versioni chiave con timestamp anziché 17 registrazioni minute, e che ogni versione sia ripristinabile con un click.
      url: '#day-3'
    - name: Giorno 5 — prova la cancellazione
      text: Cancella di proposito un file di prova non importante, apri la lista «File eliminati» di Keeply e verifica che il file sia ancora lì e recuperabile con «Ripristina», a conferma che la cancellazione è indipendente dal cestino di sistema.
      url: '#day-5'
    - name: Giorno 7 — valutazione complessiva
      text: "Ripassa le tre cose della settimana: Keeply ha visto il file aggiunto, ha tenuto un numero ragionevole di versioni, dopo la cancellazione lo hai recuperato. Se tutte e tre le risposte sono «sì» continua a usarlo; se anche solo una non torna, rimuovilo subito."
      url: '#day-7'
---
