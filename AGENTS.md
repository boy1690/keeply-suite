...
> Phase 4 (AGENTS.md split) will expand the per-app rules — current state is post-Phase-2 (subtrees imported).

---

## Apps

| Dir | Stack | Domain | Source-of-truth AGENTS.md |
|-----|-------|--------|---------------------------|
| `apps/blog/` | Hugo 0.160.1 extended | `blog.keeply.work` | [apps/blog/AGENTS.md](apps/blog/AGENTS.md) |
| `apps/website/` | Vanilla HTML + Node build (Tailwind + custom build.js + fingerprint + SRI) | `keeply.work` (apex) | [apps/website/AGENTS.md](apps/website/AGENTS.md) |
...
### Push policy
- Every `git push` requires explicit user authorization (inherited from global `~/.Codex/AGENTS.md`)
...
- This replaces the old "cross-repo boundary" rule from the pre-merge `keeply-blog/AGENTS.md`
...
| Phase | Status | Note |
|-------|--------|------|
| 1. umbrella structure | ✅ done | README + AGENTS.md + .gitignore + initial commit |
| 2. subtree import (blog + website) | ✅ done | full git history preserved (no `--squash`) |
| 4. AGENTS.md split | ⏳ pending | will expand this root AGENTS.md + populate `apps/{blog,website}/AGENTS.md` |
...
| 6. hooks / skills / scripts path updates | ⏳ pending | `.Codex/hooks/keeply-mock-audit.py` paths → `apps/blog/content/`; skill `keeply-mock-ui` `BLOG_ROOT` env var |
...
When Codex session opens with working dir `D:/tools/doing/keeply-suite/`:
- Project memory path: `~/.Codex/projects/d--tools-doing-keeply-suite/memory/`
