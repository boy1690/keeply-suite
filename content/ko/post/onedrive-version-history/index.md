...
description: "Microsoft Learn에 명시되어 있습니다: OneDrive는 파일당 최대 500개의 주요 버전을 보관합니다. 휴지통 보존은 개인 30일 / 회사 93일. 자동 복구는 별개의 메커니즘. 이 글은 3가지 메커니즘을 분해하고 Keeply Release freeze가 cap 초과 시나리오를 어떻게 해결하는지 설명합니다."
...
image_alt_data: "OneDrive 버전 기록 500 cap + 30일 휴지통 + AutoRecover 3가지 메커니즘 다이어그램, Microsoft Learn 공식 URL 인용"
faq_schema:
  ...
  - q: OneDrive 버전 기록과 AutoRecover는 같나요?
    a: "다릅니다. 버전 기록은 OneDrive 클라우드 쪽 버전별 보존입니다. AutoRecover는 Office 데스크톱 클라이언트의 비상 저장(기본 10분 간격)으로 앱이 충돌하거나 강제 종료할 때만 작동합니다. 완전히 다른 저장 계층에 있습니다."
  - q: 왜 6개월 전 OneDrive 파일을 찾을 수 없나요?
    a: "두 가지 가능성. (a) 500 버전 cap을 초과하여 자동 삭제됨, (b) 버전 기록 대신 휴지통을 검색했는데 30일 창이 이미 닫힘. 헤비 유저(하루 3회 이상 저장)는 7-8개월 안에 cap에 도달합니다."
  - q: 500 버전 초과 후 어떻게 되나요?
    a: "OneDrive가 조용히 가장 오래된 버전을 삭제하여 새 버전을 위한 공간을 확보합니다. 경고 없음, 아카이브 마이그레이션 없음, 오래된 버전을 유지할 옵션 없음. Release-freeze 메커니즘과 버전 cap이 없는 도구(예: Keeply)가 이 갭을 메웁니다."
  - q: Keeply는 OneDrive와 충돌하나요?
    a: "충돌하지 않습니다. Keeply는 OneDrive와 병행 실행 — 작업 폴더를 감시하고 OneDrive의 cap과 독립된 자체 타임라인을 유지합니다. OneDrive는 협업 동기화용으로 유지하고, Keeply는 노트와 Release freeze가 있는 무제한 파일별 버전 기록용으로 사용하세요."
---
...
이것은 버그가 아닙니다. [Microsoft Learn](https://learn.microsoft.com/en-us/sharepoint/document-library-version-history-limits)이 처음부터 명시한 500 주요 버전 상한입니다. 그러나 90%의 OneDrive 버전 기록 튜토리얼은 **사용 방법**을 가르치고 **어디서 멈추는지**는 가르치지 않습니다. 이 글은 그 갭을 메웁니다 — 자주 혼동되는 3가지 OneDrive 메커니즘(버전 기록 500 cap / 휴지통 30일 창 / AutoRecover)을 분해하고, [Keeply](https://keeply.work)가 cap 초과 시나리오를 어떻게 처리하는지 보여줍니다.

## 목차

1. [Keeply가 OneDrive 기록을 501번째 저장에서 사라지지 않게 하는 방법](#keeply-timeline)
2. [OneDrive의 3가지 메커니즘: 500 / 30일 / AutoRecover — 서로 다른 것들](#three-mechanisms)
3. [500 버전 cap: Microsoft 자체 숫자와 도달하는 시점](#500-cap)
4. [휴지통 30 / 93일: 삭제 시간 창, 버전 기록이 아님](#recycle-bin)
5. [AutoRecover: Office 충돌 버퍼, 버전 기록과 완전히 분리됨](#autorecover)
6. [Keeply가 갭을 메웁니다: cap 후의 Release freeze + 파일별 노트](#keeply-fills)
...
"Client signed v2.3 — board approved"를 한 줄 쓰고 버전을 저장합니다. 내년 3월 타임라인에서 태그를 보면 바로 찾을 수 있습니다 — OneDrive 500 cap에 영향받지 않고 자동 삭제되지 않습니다.
...
| **AutoRecover** | Office 클라이언트 충돌 버퍼 | 기본 10분 간격 | 앱 충돌 / 강제 종료 |

3가지 다른 것 — 하나로 혼동하면 잘못된 계층을 찾게 됩니다. "6개월 전 파일을 찾을 수 없어요"는 버전 기록 500-cap이거나, 휴지통 30일 창이 만료되었거나, AutoRecover가 오래 전에 덮어쓰였을 수 있습니다. 문제마다 다른 해결책이 필요합니다.

## 500 버전 cap: Microsoft 자체 숫자 {#500-cap}
...
- **컨설턴트** — 제안서를 하루 3회 저장 × 22 영업일 = 월 ~66 버전 → **7-8개월** 안에 cap
- **디자이너** — 디자인 파일을 하루 5-8회 저장 → **3-4개월** 안에 cap
- **작가 / 변호사** — 원고를 하루 10회 이상 저장 → **3개월 미만**에 cap

저장 빈도가 높고 + 다개월 프로젝트 = cap 도달 가능성 높음. Microsoft는 경고하지 않습니다. UI도 표시하지 않습니다. 찾으러 가서야 알게 됩니다.
...
버전 기록과 휴지통은 **별개의 두 시스템**입니다. `proposal.docx`를 v200에서 v201로 수정 → 이전 버전이 버전 기록으로 들어갑니다(휴지통이 아님). `proposal.docx`를 삭제 → 전체 파일이 휴지통으로 들어갑니다(버전 기록 포함). 전자는 500 cap에, 후자는 30/93일 cap에 도달합니다.

## AutoRecover ≠ 버전 기록 {#autorecover}

Word / Excel / PowerPoint 데스크톱 클라이언트의 AutoRecover는 `.asd` 임시 파일을 저장합니다 — 기본 **10분 간격** — 다음 경우에만 유용:
...
## Keeply가 갭을 메웁니다 — OneDrive cap 후 {#keeply-fills}

Tina의 `proposal.docx`가 500 cap에 도달했습니다. 고객이 갑자기 8개월 전 제안서를 원합니다 — OneDrive에는 더 이상 없습니다.

[Keeply](https://keeply.work)에서는 3가지가 하나의 도구에 있습니다:

- **Release freeze**: 2월 14일 고객이 서명할 때 Tina가 "버전 저장"을 누르고 "Client signed v2.3"으로 태그 지정 — 그 버전이 별도의 스냅샷으로 동결되어 이후 500회 저장에도 덮어쓰이지 않고 영구 보존. OneDrive 500-cap이 적용되지 않습니다.
- **파일별 노트**: 각 버전에 1-2줄의 노트를 작성할 수 있습니다. 3개월 후 Tina가 타임라인에서 "CFO 3차 수정", "고객 서명", "이사회 준비" 태그를 보면 — 12개의 `_FINAL` 파일 이름을 추측할 필요 없습니다.
- **크로스 도구 이식성**: Keeply는 OneDrive에 의존하지 않습니다. Dropbox / NAS / 새 노트북으로 전환해도 — 타임라인은 로컬 + Keeply 자체 백업 위치에 남습니다. 어떤 클라우드 벤더의 cap도 당신을 가두지 않습니다.
...
**하루 1회 미만 저장, 개인 사용**. `notes.docx`를 일주일에 한 번만 편집한다면 — OneDrive 500 cap에 10년이 지나도 도달하지 않습니다. Keeply는 시급하지 않습니다.
...
버전 기록 자체는 시간 제한 없음 (500-cap에 의해 제한됨). 시간 제한이 있는 것은 휴지통: 개인 30일, 회사 93일.

**Q3: OneDrive 버전 기록과 AutoRecover는 같나요?**

다릅니다. 버전 기록은 OneDrive 클라우드 쪽 버전별 보존, AutoRecover는 Office 데스크톱 충돌 버퍼 (10분 간격). 다른 저장 계층.

**Q4: 왜 6개월 전 OneDrive 파일을 찾을 수 없나요?**

두 가지 가능성: (a) 500-cap 초과, 자동 삭제됨; (b) 휴지통을 대신 검색했는데 30일 창이 닫힘. 헤비 유저는 7-8개월 안에 cap 도달.

**Q5: 500 버전 초과 후 어떻게 되나요?**

OneDrive가 조용히 가장 오래된 것을 삭제, 경고 없음. 해결하려면 cap이 없는 도구가 필요 — 예를 들어 [Keeply](https://keeply.work) Release freeze.

**Q6: Keeply는 OneDrive와 충돌하나요?**

충돌하지 않습니다, 병행 실행. OneDrive는 협업 동기화용, Keeply는 무제한 파일별 버전 기록 + 노트 + Release freeze.
...
Tina의 `proposal.docx`가 OneDrive에서 500 cap에 도달했습니다. 고객이 다음 달 8개월 전 제안서를 원합니다 — Microsoft 자체 규칙에 따라 공식 문서대로 삭제되었습니다.
