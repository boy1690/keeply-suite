...
per-save 계층 추가가 overkill 인 몇 가지 상황:
...
**클라우드 동기화가 중요 파일을 커버**. 중요한 것 모두가 OneDrive / Dropbox / Google Drive 에 있고 보관 기간 내라면、이미 클라우드 버전 기록에 일종의 의도 기반 계층이 있다(다만 cap——[버전 기록의 절벽](/ko/post/cloud-version-history-cliff/) 참조).
