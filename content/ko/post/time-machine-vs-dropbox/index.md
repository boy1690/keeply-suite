...
모든 비교 글이 놓치는 것：**파일별 의도적 저장의 기록、시간 상한 없음、계수 상한 없음、특정 저장을 「영원히 남는 마일스톤」으로 표시할 수 있는 능력**.
...
| 기기 간 클라우드 동기화 | ❌ | ✅ |
| **파일 수준 의도적 버전 기록** | ⚠️ 디스크 수준만、파일 수준 아님 | ⚠️ 30 일 상한(유료 180) |
...
Dropbox 에는 파일 수준 버전이 있지만 무료 30 일 상한、유료 180 또는 365 일. 상한 을 넘으면 그 파일 수준 기록은 사라집니다.
...
- **시간 상한 없음**——2 년 전 버전은 어제 버전만큼 쉽게 꺼낼 수 있음
- **계수 상한 없음**——500 번 저장한 후에도 초기 것이 살아 있음
...
[Keeply](https://keeply.work) 는 이 세 번째 계층의 한 구현입니다. 로컬에서 돌고、추가한 폴더를 watch 하고、의도적인 저장마다 잡아냅니다、상한 없음. Release 기능으로 특정 버전을 마일스톤으로 동결할 수 있습니다.
...
저 ★ 마커가「화요일 오후에 의도적으로 저장한 그 버전」을 돌려줍니다——Dropbox 의 30 일 상한、Time Machine 의 시간 단위 솎아내기、그리고 정확히 어느 날이었는지 잊은 당신의 기억을 모두 견디고.
