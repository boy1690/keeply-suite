---
title: "Photoshop 자동 저장은 크래시는 살려도, 덮어쓴 버전은 못 살린다"
...
덮어쓴 그 레이어가, 지금 손에 남아 있는 유일한 v2다. 너는 다급하게 "photoshop autosave location"을 구글링한다 — 어딘가에 Photoshop이 몰래 사본을 두고 있을 거라고 믿으면서, 자동 저장 폴더를 연다. 지난주 화요일 파일이 하나 있다. 오늘 것은 아무것도 없다.
...
빈 폴더 앞에서 디자이너들은 보통 두 가지를 한다. "photoshop autosave 어디"를 한 번 더 구글링하고, 그다음 폴더를 십 분간 멍하니 바라본다. 둘 다 헛수고다. 자동 저장은 처음부터 다른 메커니즘이기 때문이다 — Photoshop이 자기 자신을 위해 준비한 비상 낙하산이고, 낙하산이 펴지는 건 "프로그램이나 시스템이 갑자기 죽는 순간"이며, 낙하산 아래에 서 있는 건 Photoshop 자신이지 너의 버전 기록이 아니다.
...
직접 폴더를 뒤져 확인하고 싶다면? [Adobe 공식 문서가 플랫폼별 정확한 경로를 안내한다](https://helpx.adobe.com/photoshop/using/auto-save-recovery-background-save.html): Mac의 `~/Documents/Adobe/AutoRecover/`, Windows의 `%AppData%/Adobe/Adobe Photoshop {version}/AutoRecover/`. 이전 세션의 오래된 `.psb` 파일이 아직 남아 있을 수도 있다. 하지만 오늘의 작업은 거기에 한 번도 쓰인 적이 없어서, 복원할 수가 없다.
