...
여기가 핵심: **git 메커니즘 은 써도 됨, UI 에 노출 안 함**. 비개발자용 버전 관리의 핵심.

이유:

- git 의 delta storage / merge / branching 은 기술적으로 우수 (증명됨)
- 문제는 git 의 UI/CLI 가 개발자 향, 비개발자에게 혼란
- 해결: **git 메커니즘 + non-developer UI = 비개발자용 버전 관리**

구체적 예: Keeply 의 ADR-001 은 「UI 에 저장 시점/branch/HEAD 안 나옴」을 규정. git terminology 를 office 언어로 wrap:

- 「버전 저장」 = 「저장 시점」
- 「버전 기록」 = 「git log」
- 「복원」 = 「checkout」

맞아요, 이게 핵심 입니다. Atlassian, GitHub, Stack Overflow 모두 개발자 에게 말합니다. 「메커니즘 + UI 분리」 각도는 아무도 잡지 않습니다.
...
시스템 레벨 파일 복원, 매시간 자동 스냅샷. **장점**: 파일 단위 UI, CLI 불필요, 이진 파일 지원. **단점**: Mac only, 복원은 타임라인 UI 로 일부 불편, milestone freeze 없음. **적합**: macOS 개인 사용자, 돌발 복원 only.

### Option B: Dropbox 버전 기록 (30 일 제한 버전)

30 일 내 버전 자동 보존, UI 는 파일 우클릭→「이전 버전」으로 복원. **장점**: 크로스 플랫폼, 공유 편리. **단점**: 30 일 후 사라짐, 셀-level 차이 없음, 충돌 사본 문제 ([다른 글 참조](/ko/post/dropbox-conflicted-copy/)). **적합**: 30 일 내 collaborative editing.

### Option C: Keeply

git2 엔진 + ADR-001 git terminology 숨김 UI. **장점**: 파일 단위 UI, CLI 불필요, 이진 LFS 자동, 시간 제한 없음, 릴리스 milestone. **단점**: 데스크톱 우선 (mobile 약함), 즉시 동기화 강점 아님, 실시간 협업 부적합. **적합**: 비개발자 개인 / SMB, 장기 기록 필요, binary 중시.
...
- **Real developer**: CLI access 원함, git 기록 그래프 보고 싶음, Keeply UI 너무 숨김
- **대기업**: SSO / Active Directory 통합 없음
- **Mobile-first**: Keeply 는 데스크톱 우선
- **실시간 협업**: Microsoft 365 공동 편집 / Google Docs 가 더 강함
