...
완전한 파일 기록 계층 는 기술적으로 사소한 입니다. macOS Time Machine 이 2007 년에 업계 전체에 시범을 보였습니다. Microsoft 는 할 수 있다, Microsoft 가 안 하기로 선택했을 뿐.
...
「이 버전은 월말 결산 v3」「이 버전은 Q2 close」를 능동적으로 표시. 표시된 지점은 그 이후 어떻게 바뀌든 남습니다. **예**: Git tag (개발자 전용), Keeply 릴리스 (내장, UI 에 git 용어 없음). **제한 #2 의 연장 시나리오 해결**: 분기 리뷰에서도 당시의 버전을 찾을 수 있음.

### Design C: 버전 내용 검색

기록상 어느 버전에서든 셀 내용 검색 (파일명만이 아님). **예**: Keeply spec 049 버전-search 로 기록 버전의 셀 내용 검색. **제한 #4 의 일부 해결**: 셀 단위 차이 는 아니지만, 「그 100 원 숫자가 마지막에 나타난 건 어느 버전인가」를 찾을 수 있음.
