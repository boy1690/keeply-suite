...
> AI 에이전트가 앞서 달려나가서 코드가 안 돌아가요. Keeply Timeline을 여세요. 마지막으로 작동하던 버전이 그대로 거기 있어요.

## 글 목차

1. [AI 오버슛의 순간은 어떻게 보일까?](#ai-overshoot)
2. [동작 한 번: Timeline을 열고, 마지막 작동 지점을 클릭](#one-action)
...
## 동작 한 번: Timeline을 열고, 마지막 작동 지점을 클릭 {#one-action}

### 1단계: Keeply Timeline 열기

왼쪽 사이드바의 첫 번째 탭. 오늘의 모든 변경이 시간순으로 보여요.

### 2단계: 코드가 「아직 돌아가던」 마지막 지점 찾기

Timeline의 각 항목은 Keeply의 자동 저장점이거나, 직접 표시한 순간이에요. 각 지점을 열어 안의 변경을 보고, 「그때는 테스트 OK였다」고 기억하는 버전을 찾으세요.

보통 30~60분 전이에요. AI가 옆길로 빠지기 시작하기 직전의 마지막 테스트.

![Keeply Timeline 확대: 각 파일 노트가 타임스탬프 + 변경된 줄 + 그 시점의 테스트 기록을 보여줌](image-2.svg)
...
다음에 오버슛이 나면, Timeline을 열고 마지막 항목을 클릭하세요. **30초면 문제 종결.**
