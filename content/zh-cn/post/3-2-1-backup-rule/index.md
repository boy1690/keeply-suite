...
faq_schema:
  ...
  - q: Keeply 已经是 3-2-1 吗？
    a: Keeply 内建 3-2-1-shape 位置层（本机工作副本 + 正本 + 备援位置），加上每次保存自动留版的版本历史，以及「发行版」冻结机制（把某次成果标成「给客户 v2.3 那版」、不被后续保存覆盖）。一套工具同时做空间冗余 + 时间冗余 + 发行版冻结。
---
...
**3-2-1 备份原则**是必要的：三份文件、两种媒介、一份异地，防的是硬盘损毁、机房失火这类灾难。但它从设计就没处理**操作失误**：你自己覆盖、同事改错、云端同步把错的版本传到三个位置，3-2-1 都救不了。本文拆解 3-2-1 防什么、不防什么，以及 Keeply 怎么把那一层补上。
...
更重要的是「时间」这层也补上了。每 30 分钟背景自动存一次、加上他重要时刻可以点「保存版本」、写笔记。半年后客户打来要某一版，他翻时间轴：

```
Keeply 时间轴 — proposal.psd

3 个月前 — 2 月 14 日（客户签约日）
─────────────────────────────────
● 16:42   proposal.psd   ★「给客户 v2.3 — 签约版」 ← 亲手存的（发行版）
● 14:30   proposal.psd   （自动保存）
● 11:20   proposal.psd   ★「修完客户第二轮反馈」 ← 亲手存的

上礼拜 — 5 月 8 日
─────────────────────────────────
● 17:00   proposal.psd   （自动保存，A 先生改错后）
● 14:22   proposal.psd   （自动保存）
```

「给客户 v2.3 — 签约版」那一行自己一行、有他写的笔记。点开就是 3 个月前客户看的那一版。不用翻 12 个 `_FINAL` 文件猜哪个是哪个。
