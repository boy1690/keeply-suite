...
description: "备份顾整个磁盘、云端顾最新一份、Keeply 顾每次变动的历史——三件不同的事。本文拆解这三类工具各自存什么、解什么，以及为什么最常见的「我改错了」情境前两个都救不了。"
...
但如果你只想找回 `proposal.docx` 在周四 10:23 改之前的版本，备份做得到，但你要先还原整个 快照 才能挑出那个文件。**这不是它设计来解的问题**。

![Time Machine 整个磁盘 快照 vs Keeply 单档时间轴概念对比](image-2.svg)
...
「跟 Time Machine 不一样。Time Machine 顾整台电脑的 快照。Keeply 顾每个文件的历史轴。**我两个都用**。」
