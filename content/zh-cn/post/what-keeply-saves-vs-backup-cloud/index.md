...
faq_schema:
  ...
  - q: OneDrive / Dropbox 不是也有版本历史吗？
    a: 有、但只保留 30 天（Dropbox 标准方案、Google Drive、OneDrive 都是这条规则），而且每一版只有时间戳、没有「为什么存」的笔记。3 个月后想找回「业主确认版」那一版，云端通常已经没了——就算没被删，也只有 14:00、15:00 一排时间戳要你自己猜。
...
它们也有版本历史。但**通常只保留 30 天**——Dropbox 标准方案、Google Drive、OneDrive 都这样，超过就删掉。
...
A 先生装的 Keeply 就是补这层的。它在背景跑、半年后翻回来你看到的是这个：

```
Keeply 时间轴 — proposal.docx

5 月 13 日 — 星期二
─────────────────────────────────
● 14:50   proposal.docx   ★「会议后加结论」 ← 你亲手存的
● 14:30   proposal.docx   （自动保存）
● 11:50   proposal.docx   ★「业主第一次反馈后」 ← 你亲手存的
● 10:12   proposal.docx   （自动保存，第一次写）
```
