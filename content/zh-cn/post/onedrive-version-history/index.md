...
相关详细请看 [Photoshop 自动保存 不是 version history](/zh-cn/post/photoshop-autosave-not-version-history/)——Adobe 那层同款混淆机制。
...
客户来信当下、Tina 在 Keeply 时间轴找到 2 月 14 日那笔「客户签 v2.3」、按右键还原——跳出对话框：

![Keeply 还原文件对话框：要把 proposal.docx 还原到「客户签约 v2.3 — 5/4」吗？还原前自动建立当前版本快照、可随时 Undo](restore-file-dialog.svg)

她按「还原」。3 秒钟 `proposal.docx` 回到 2 月那版的状态；当前版本被自动快照、想反悔随时 Undo。OneDrive 留给同步协作（这是它强项）、Keeply 给她 无上限 的单档版本历史。
