...
description: "iCloud、Dropbox、OneDrive、Google Drive 比較文都在比容量、價格、分享 UX，沒人比版本歷史 retention。4 家其實共通有 cap（時間制或計數制），iCloud 甚至沒對外公布通用檔的政策。代價在 3 個月後出現——客戶要那版你叫不出來。"
...
image_alt_data: "iCloud Drive、Dropbox、OneDrive、Google Drive 四家雲端的版本歷史 retention 並排比較——Apple 沒公布、Dropbox 30/180/365 天、OneDrive 500 版本計數、Google Drive 30 天或 100 版本——揭露 4 家共通有 cap，比較文從沒列出來"
faq_schema:
  ...
  - q: Keeply 是雲端儲存的替代品嗎？
    a: 不是。Keeply 是雲端同步之上的另一層工具——本地保留每次存檔，無時間限制、無計數限制。雲端負責同步跟異地副本，Keeply 負責版本歷史。你不需要換工具，是疊一層上去。
---
...
| 雲端 | 通用檔版本歷史 | Retention 形狀 | 實際 cap |
|---|---|---|---|
...
| **OneDrive** | ✅ 有 | 計數制 + 刪除視窗 | [保留 500 major versions](https://learn.microsoft.com/en-us/sharepoint/document-library-version-history-limits)；回收筒 personal 30 天 / business 93 天 |
...
3 家有 expose 版本歷史的雲端各自用完全不同的 cap。

**時間制（Dropbox）**——你拿到一個視窗。30 / 180 / 365 天。視窗外的版本不管有幾個都消失。兩個月前動一次的檔案，跟兩個月前動五十次的檔案，下場一樣：都沒了。

**計數制（OneDrive）**——你拿到的是 slot 數。保留 500 major versions。超過 500，最舊的版本被刪挪空間給新的。可能是兩年累積 500 個版本，也可能是一週內就改了 500 次，1 月看過的那版 2 月就消失。
...
4 家共通的盲點：**每家都有 cap。Cap 形狀不一樣。比較文從沒告訴你哪個形狀符合你的工作。**
...
Reframe 一下：你不換雲端就能解這個問題。你的雲端做同步沒問題。少的那一塊是**另一層**——檔案層級的版本歷史，無時間 cap、每次存檔自動觸發。

具體說：

- **雲端（4 家任一）**負責同步 + 異地副本
- **版本歷史層（Keeply 或同類）**負責每次存檔，無時間 cap、無計數 cap、不需要在存檔當下決定要不要「Keep forever」
...
Keeply 是這層的 reference 實作：每次存檔本地保留，無時間 cap、無計數 cap，加上「Release」凍結機制——把某個版本標成「這版送給客戶」，那個 snapshot 永遠存在，後續存五十次也蓋不掉。兩個月前的版本，回溯約 2 個點擊。
...
65 天前那版上面的 Release 標記表示它過了 OneDrive 500 版本 cap、過了 Dropbox 30 天視窗、過了 Google Drive 100 版本計數，還是能拉回來——因為 Keeply 不像雲端那樣套 cap。
