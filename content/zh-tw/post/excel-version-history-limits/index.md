...
> Excel 版本歷史按鈕變灰、只回 1-2 版？不是 錯誤、是 Microsoft 把 AutoSave 當 OneDrive 訂閱誘餌設計。
...
第 4 條最有感。Excel 版本歷史只給你「整個檔」回滾、不告訴你 F14 那格到底改了什麼。Keeply 的版本比對直接秀儲存格層級差異：

![Keeply 版本比對：monthly_close.xlsx 應收帳款工作表 F14 公式從 5% 改成 7%、F22 小計從 1,284,500 變 1,309,720](diff-viewer.svg)

看到 F14 公式是 5% 改成 7%、就知道「啊、是應收率上次調高了」、不用再開兩個 Excel 視窗用眼睛掃。每個限制都是 Microsoft 工程上**故意不解**的選擇、不是技術做不到。下一段講為什麼。
...
對啊、這就是讓人煩的地方。你撞到的不是 錯誤、是付費牆。只是 Microsoft 不會這樣講。版本歷史對使用者是**檔案安全網**；對 Microsoft 是**訂閱上鉤餌**。同一個功能兩個角色、誰決定行為？決定的人不是你。
