---
title: "【2026 檔案管理】消失的 Excel 資料：星期二 14:32，共同編輯吃掉的 Sheet 怎麼從 4 層救援"
description: "星期二 14:32，提案資料的 Sheet 在共同編輯中消失。秒級檢驗 OneDrive、SharePoint 版本歷史、Time Machine、還原軟體 4 層救援的事故報告書。"
...
> 星期二下午 14 點 32 分。業務的鈴木先生（**合成案例**）打開 OneDrive 上的「提案_客戶A_v3.xlsx」，這份檔他從星期一就在準備。明天早上 10 點要提案，剩 19 小時 28 分。檔案打開那一刻，畫面凍住。Sheet「業績實績」是空的。分頁還在、欄位 A 到 AZ 都還在、行號 1 到 250 也在，可是每一格都是白的。連動的 Sheet「報價」合計欄也整排 `#REF!`。OneDrive 角落是綠色勾。Excel 上面有個小字「另有 1 人正在編輯」。中午午休的時候，後輩田中開過這個檔。
...
14 點 32 分 17 秒，鈴木打開「提案_客戶A_v3.xlsx」。Excel 啟動，他點 Sheet「業績實績」。畫面讀取中，0.4 秒後，整片白。Sheet 分頁還在、欄頭 A 到 AZ 都在、行號 1 到 250 也在，可是每一格都是空的。Excel 上面寫「另有 1 人正在編輯」，旁邊是後輩田中的頭像。15 秒前田中還開著這個檔。鈴木看到的，就是田中中午 14:32 還在編的雲端版本。

事故的細節：

- **昨天 17:50**：鈴木把 Sheet「業績實績」填好 250 筆客戶實績，關檔。
- **今天 12:31**：田中（後輩）開檔（鈴木有跟他說「明天要提案」）。
- **今天 12:46**：田中對 Sheet「業績實績」右鍵，按「刪除工作表」（誤操作，事後解釋「以為刪的是測試用的 Sheet」）。
...
- **今天 14:32**：鈴木開檔，雲端那個狀態（Sheet 已刪）被同步下來。
...
14 點 32 分 47 秒，鈴木想搞清楚狀況，先看 OneDrive 圖示。綠色勾。全部同步，沒錯誤。真的嗎？

OneDrive 的同步勾代表「本機檔案跟雲端一致」，不代表「資料沒壞」。田中 12:46:03 刪了 Sheet，刪除被推上雲端，鈴木的電腦（在辦公室，下午才開機）還沒同步。14:32 鈴木一開檔，OneDrive 立刻拉雲端狀態，把本機檔案覆蓋成「Sheet 已刪」。同步勾顯示成功。

「已同步」不等於「你的工作安全」。在共同編輯模式下，別人的 delete 也會同步進來。

## T+5 分鐘：SharePoint 版本歷史打開來，找不到的東西

14 點 37 分，鈴木用瀏覽器打開 OneDrive，對檔案右鍵，選「版本歷史」。列表跳出來：v8（12:46，田中）/ v7（昨天 17:50，鈴木）/ v6（昨天 17:30，鈴木）...
...
15 點 32 分，鈴木放棄了，把 Excel 關掉。他想「再開一次試試另一個還原版本」。

開回去發現：Ctrl+Z 按不了。「復原上一步」是灰的。Excel 的 undo stack 是 **per-session**，檔案一關，全部的 undo 紀錄（不管是自己的操作，還是共同編輯對方的操作顯示）都重置。本來可以拿來「復原田中刪 Sheet」的編輯 session，在 14:46 鈴木關檔那一秒就消失了。

undo stack 在記憶體裡，per-session 結構，不會存進檔案、也不會上雲。這是 Microsoft Office 全產品共通的規格。

## T+24 小時：Time Machine 拍到的是雲端狀態

隔天早上 9 點，鈴木想到「公司的 Mac 應該有 Time Machine」，發信給 IT 部。30 分鐘後回應：「Time Machine 快照有，每小時自動拍」。
...
事故發生後 4 小時。鈴木什麼都還沒救回來。

## 平行宇宙：鈴木的電腦裡裝了 Keeply，14:32 會怎樣

如果，在平行宇宙裡，鈴木的電腦裝了 Keeply，14 點 32 分那一刻，會發生什麼事？

Keeply 在本機保管庫保存獨立的快照，跟 OneDrive 走完全不同的路徑、不同的儲存空間。Keeply 不知道「Office 365 共同編輯」這件事，也因為不知道，田中的 delete 不會反映到 Keeply 的保管庫裡。

鈴木的 Keeply 設成 15 分鐘間隔背景自動儲存。昨天 17:50 鈴木關檔後，18:00 Keeply 拍了最後一張自動儲存：「業績實績」250 筆、「報價」公式正常。今天 12:46 田中的 delete 發生在雲端側，鈴木的電腦在辦公室、下班後關機，Keeply 不會動。14:32 鈴木開機，OneDrive 同步雲端狀態下來，可是 Keeply 的保管庫跟 OneDrive 不在同一條路徑上，不受影響。

14 點 33 分，鈴木打開 Keeply：
...
打開檔案確認，「業績實績」250 筆 ✅、「報價」公式 ✅。鈴木傳 LINE 給田中：「測試的請改用新檔案，這個才是本尊」。30 秒解決。

Keeply 在背景自動儲存（間隔可選 15 / 30 / 60 分鐘，預設 30 分鐘；鈴木這台設 15 分鐘）+ 重要節點你可以主動按「儲存版本」按鈕 + 每張快照分別存在保管庫，不會互相覆蓋。整個流程不經過共同編輯、不經過雲端同步，是本機磁碟上的另一個世界。
...
1. **檔案放在共用網路磁碟、鈴木本機沒副本**。Keeply 只看本機檔案，不會去監看共用磁碟上別人改了什麼。共用磁碟要團隊另外建一台 Keeply 鏡像保管庫。
2. **田中直接登入鈴木的電腦（遠端桌面之類）改檔再刪**。這時 delete 是本機事件，Keeply 會記錄。要還原就是從 Keeply 保管庫拉回來，可是那一秒同時推上雲端，遠端同步起來會比較複雜。
