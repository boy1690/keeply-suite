...
你說「修那個 error」。AI 動了 5 個檔案，改到 設定，加了一個你沒問過的 輔助工具 功能。跑，更多 error。

![AI agent 對話視窗 vs 你電腦上實際被改的檔案數對比](image-1.svg)

這時候 AI 還在自信地修。**它不會主動說「我可能寫崩了」**。

它的記憶只有當前 情境 視窗。**它不知道 5 個 提示 之前你的 程式碼 是好的**。但你電腦上的檔案知道。只要有人記住。
...
左側 側邊欄 第一個 tab。你會看到今天的所有變動，按時間排。
...
整個資料夾在 30 秒內回到那個時間點的狀態。**所有檔案、所有目錄結構、所有 設定 全部一起回去**。不只是一個檔案。

包括 AI 偷加的 輔助工具 功能、改過的 設定、不該被動的 .env。**全部回去**。
...
AI agent 設計成**往前推進**的。它收到 提示，產出 edit。它不會主動回望「我剛才那一輪是不是把整個專案變糟了」。

這個責任不在 AI 身上。是它的架構限制。

責任在你：**你需要 安全 網 在背景跑**。AI 衝多遠都行，因為你叫得回它。

不是 Keeply 要取代你寫 程式碼。是你 vibe coding 的時候，不該靠記憶力回頭。記憶力會輸給 AI 改檔案的速度。
