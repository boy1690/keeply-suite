...
faq_schema:
  ...
  - q: 兩個都不解的第三軸是什麼？
    a: 檔案層級的故意存檔版本歷史——無時間 cap、無計數 cap——把每次 Cmd+S 都當成一個獨立可回溯的點，加上「標記某版為『這是送給客戶的那版』」讓它永遠不被覆蓋。像 Keeply 這類工具就是在磁碟備份跟雲端同步之外，獨立做這第三層。
---
...
每篇比較文都漏的這件事：**每個檔案故意存檔的紀錄，無時間 cap、無計數 cap，加上能標記某次存檔為「這個版本永遠不准被蓋掉」**。
...
- **無時間 cap**——2 年前的版本跟昨天的一樣好叫出來
- **無計數 cap**——500 次存檔之後，最早的還在
- **「Release」或「Milestone」標記**——把某次存檔標成「這是我 3 月 8 號送給客戶的版本」，永遠不被覆蓋
- **跟 Time Machine 跟 Dropbox 共存**——不取代它們，活在第三軸上

[Keeply](https://keeply.work) 是這層的一個實作。本機跑、watch 你加進去的資料夾、每次故意存檔都抓、無 cap。Release 功能讓你把某版凍結成里程碑。
...
那個 ★ 標記就是你要的「禮拜二下午故意存的那版」——撐過 Dropbox 30 天 cap、撐過 Time Machine hourly thinning、撐過你自己忘記是哪一天的記憶。
...
**你在需要不可變存檔的合規產業**：版本歷史不是 compliance archive。如果 GDPR / HIPAA / SOX 要求「這個版本創建後不能再被修改」，你需要 archive 級工具（Veeam、Acronis），不是 Time Machine + Dropbox + 版本歷史。
