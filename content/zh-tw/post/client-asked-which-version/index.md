...
我做 Keeply 客戶訪談時，這通晚上 11:23 的 LINE 是我聽過最多次的場景。
...
| OneDrive 版本歷史 | 預設約 [500 個版本](https://support.microsoft.com/en-us/office/enable-and-configure-versioning-for-a-list-or-library-1555d642-23ee-446a-990a-bcab618c7a37) | 超過 500 自動 清除 最舊 | 短期回滾 |
...
我自己待在工地現場那幾年，每個檔案版本都決定最後交付的成果。交付版找不到，等於在考驗管理者記憶的極限。

## 為什麼這些機制守不到 3 個月後？

這裡要拆一個沒人明講的差別：**儲存層** vs **工具層**。

軟體內建的版本歷史活在**儲存層**。它存在的目的是「最近一次寫入失敗就回滾」，所以 保留期 設得短。從檔案關閉清除到 500 個版本上限，這些設計參考的是「平均使用者一個月內回頭找的次數」。3 個月以上不在設計目標內，清除 掉是合理。
