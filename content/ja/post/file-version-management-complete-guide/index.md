...
日常に置き換えると：3 ヶ月前のクライアント 概要 を探したい？企業プランを払っていない限り、**もう存在しない**。

この 30 日は技術的にできないわけじゃない。純粋に商業的判断。ツールはファイル履歴を、アップグレードの理由に変えた。
（Keeply はファイル履歴を、永遠に無料で提供する。）

> 2026 年 4 月、Hacker News のユーザー julianozen が投稿。父親が 2 年間触っていなかったファイルを上書きしてしまい、2 日後に救済を試みた。救えなかった。Dropbox の説明：30 日の 保持期間 window を超えている。julianozen の反応：「これは 30 日 履歴 の定義じゃない。」隣のユーザー lazide が一言：「Which is bonkers.（イカれてる）」 [完全 thread](https://news.ycombinator.com/item?id=47772260)
