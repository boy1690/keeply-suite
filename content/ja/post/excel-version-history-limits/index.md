...
金曜日の夕方 5:47、月末締めのエクセルを修正中。さっき数式を一段消して別の計算法を試したら、間違いだった。Cmd+Z で undo の上限に到達、戻れない。「ファイル > 情報 > バージョン履歴」を開く——グレーアウト。そこで気づく：このシートはデスクトップ保存、OneDrive に上がっていない。30 分の数式作業が消えた。
...
完全な file history layer は技術的に trivial です。macOS Time Machine が 2007 年に業界全体に手本を示しました。Microsoft はできる、Microsoft が選ばないだけ。

理由は商業設計：バージョン履歴は OneDrive 購読の差別化機能。デスクトップ Excel がそれ自身で完全な履歴を持ち、ローカルファイルにもあり、時間制限なしだったら、OneDrive 購読は lock-in の理由を一つ失います。
...
「このバージョンは月末締め v3」「このバージョンは Q2 close」と能動的にマーク。マークされた点は、それ以降どう変わっても残ります。**例**：Git tag（開発者専用）、Keeply Release（組み込み、UI に git 用語なし）。**制限 #2 の延長場面を解決**：四半期レビューでも当時のバージョンを見つけられる。

### Design C：バージョン内容検索

履歴上の任意のバージョンからセル内容を検索（ファイル名だけでない）。**例**：Keeply spec 049 version-search で履歴版本のセル内容検索。**制限 #4 の一部を解決**：セルレベルの差分ではないが、「あの 100 円の数字が最後に出現したのはどのバージョンか」を見つけられる。
...
- **セルレベル差分**：Keeply は「ファイル全体 v3 → v4」を表示、「セル B7 が 100 から 105 に変わった」は表示しない。セル差分は Microsoft 365 共同編集や spreadsheet diff ツールが必要。
