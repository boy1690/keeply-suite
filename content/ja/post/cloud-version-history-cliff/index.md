...
description: "iCloud、Dropbox、OneDrive、Google Drive の比較記事は容量・価格・共有 UX を測るだけで、retention は測らない。構造的な穴:4 社すべてがバージョン履歴に cap を設けている(時間制または計数制)。Apple は iCloud の方針すら公開していない。代償は 3 か月後に現れる——必要なバージョンが消えている。"
...
image_alt_data: "iCloud Drive、Dropbox、OneDrive、Google Drive 4 社のクラウドのバージョン履歴 retention を並べた比較表——Apple は非公開、Dropbox は 30/180/365 日、OneDrive は 500 バージョン計数、Google Drive は 30 日または 100 バージョン——4 社が共通の cap を持つことを明らかにする。比較記事はこのテーブルを載せない"
faq_schema:
...
| クラウド | 汎用ファイルのバージョン履歴 | Retention の形 | 実際の cap |
|---|---|---|---|
...
| **OneDrive** | ✅ あり | 計数制 + 削除ウィンドウ | [500 major version 保持](https://learn.microsoft.com/en-us/sharepoint/document-library-version-history-limits);ごみ箱 personal 30 日 / business 93 日 |
...
バージョン履歴を露出する 3 社のクラウドは、それぞれ完全に異なる cap を使います。
...
**計数制(OneDrive)**——あなたに与えられるのは枠数。500 major version 保持。500 を超えると、最古のバージョンが削除されて新しいバージョンに場所を譲ります。2 年で 500 バージョン蓄積するかもしれないし、1 週間で 500 回編集して 1 月に見たバージョンが 2 月にはもう消えているかもしれません。
...
4 社共通の盲点:**どのクラウドにも cap がある。Cap の形は違う。比較記事は、どの形があなたの仕事に合うかを一度も伝えていない。**
...
リフレームしましょう:クラウドを乗り換えなくてもこの問題は解けます。あなたのクラウドは同期には問題ありません。足りないのは**別レイヤー**——ファイル層レベルのバージョン履歴、時間 cap なし、毎回の保存で自動発動。

具体的に:

- **クラウド(4 社のうちどれでも)** は同期 + オフサイトコピーを担当
- **バージョン履歴レイヤー(Keeply または同類)** は毎回の保存、時間 cap なし、計数 cap なし、保存時に「Keep forever」を決める必要なし
...
Keeply はこのレイヤーのリファレンス実装:毎回の保存をローカルに保持、時間 cap なし、計数 cap なし、加えて「Release」凍結機構——あるバージョンを「これがクライアントに送った版」とマークすると、その snapshot は後で 50 回保存しても永遠に残ります。2 か月前のバージョンの回復は約 2 クリック。
...
65 日前のバージョンに付いた Release マークは、OneDrive の 500 バージョン cap、Dropbox の 30 日ウィンドウ、Google Drive の 100 バージョン計数、いずれも超えても引き戻せることを意味します——Keeply はクラウドのように cap を適用しないためです。
