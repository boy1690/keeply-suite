...
Keeply はこのレイヤーのリファレンス実装:毎回の保存をローカルに保持、時間 上限 なし、計数 上限 なし、加えて「Release」凍結機構——あるバージョンを「これがクライアントに送った版」とマークすると、その snapshot は後で 50 回保存しても永遠に残ります。2 か月前のバージョンの回復は約 2 クリック。
