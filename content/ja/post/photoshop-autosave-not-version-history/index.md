...
クライアントが指定した v2 を上書きしたと気づいたら、Keeply のパネルを開き、「30 分前」までスクロールして restore をクリックする——前のバージョンが、いまのファイルの隣に別ファイルとして現れる。いまの作業は触られていない。両者を視覚的に並べて比較し、v3 の色を復元した v2 にコピーする——元なら 1 時間のレイヤーやり直しだった作業が、30 秒のクリックで終わる。
