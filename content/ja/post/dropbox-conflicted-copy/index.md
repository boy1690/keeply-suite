...
### Design B：ファイル locking（原子的 check-外）

ファイルを開くとツールが自動 lock。同僚が開くと「Anna が編集中」と表示、変更不可。**例**：SharePoint、Adobe Creative Cloud Files、Bentley ProjectWise。**場面 #1 + #3 + #4 全部解決**、トレードオフ：同僚は待つ必要あり。
